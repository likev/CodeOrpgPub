 ...
 * $Author: ccalvert $
 * $Locker:  $
 * $Date: 2012/08/21 13:42:13 $
 * $Id: RPG_console_msg_to_ldm.c,v 1.6 2012/08/21 13:42:13 ccalvert Exp $
 * $Revision: 1.6 $
 * $State: Exp $
 */
...
static int  Read_console_msg_flag = YES;
...
static void Console_msg_callback_fx( int, LB_id_t );
...
int main( int argc, char *argv[] )
{
  ...
  /* Register UN events to be notified when console msg received */
  ORPGDA_write_permission( ORPGDAT_RDA_CONSOLE_MSG );
  if( ( ret = ORPGDA_UN_register( ORPGDAT_RDA_CONSOLE_MSG, LB_ANY, Console_msg_callback_fx ) ) < 0 )
  {
    LE_send_msg( GL_INFO, "ORPGDA_UN_register console msg failed (%d). Terminate.", ret );
    exit( RPG_LDM_UN_REGISTER_FAILED );
  }
  RPG_LDM_print_debug( "ORPGDA_UN_register console msg successful" );

  /* Position the read pointer for the console msg */
  ...
  /* Process console msg events */
  while( 1 )
  {
    /* Read new console messages */
    if( Read_console_msg_flag == YES )
    {
      Read_console_msg_flag = NO;
      Read_data();
    }
...
/************************************************************************
 Description: Callback if console msg is received.
 ************************************************************************/

void Console_msg_callback_fx( int data_id, LB_id_t msg_id )
{
  RPG_LDM_print_debug( "Console message callback function called" );
  Read_console_msg_flag = YES;
}
...
static void Read_data()
{
  ...
  while( break_loop == NO )
  {
    ...
    ret = ORPGDA_read( ORPGDAT_RDA_CONSOLE_MSG, &Data_buffer, LB_ALLOC_BUF, LB_NEXT );
    if( ret == LB_TO_COME )
    {
...
static void Write_to_LB()
{
  ...
  RPG_LDM_set_LDM_key( &RPG_LDM_prod_hdr, RPG_LDM_CONSOLE_MSG_PROD );

  /* Initialize RPG LDM message header. */
  RPG_LDM_msg_hdr_init( &RPG_LDM_msg_hdr );
  RPG_LDM_msg_hdr.code = RPG_LDM_CONSOLE_MSG_PROD;
  ...
  {
    RPG_LDM_print_console_msg_msg( &Data_buffer[0] );
  }

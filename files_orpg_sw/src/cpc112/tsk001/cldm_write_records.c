 ...
 * $Id: cldm_write_records.c,v 1.13 2013/01/14 15:00:00 steves Exp $
 * $Revision: 1.13 $
 * $State: Exp $
 *
 *
...
/* cldm_write_records.c - This file contains the routines used to: 
                          1. package the radials into a data block to construct 
                             LDM radial records from, 
                          2. manage the "record" file when writing to the local disk,
                          3. write the LDM record to the LDM input LB and/or
                             local "record" file.
*/
...
static void Construct_volume_hdr (char *buffer, int ldm_version);
...
static int  Populate_uncompressed_data (char *data_buffer, int data_len, 
                                        int rad_status, int ldm_version);
#ifndef LEVEL2_TOOL
static void Write_ldm_data (unsigned seqno, const char *ident, unsigned len, 
                            const char *buf);
#endif
   ...
   if ((radial_cnt == 120) || 
      ...
      ((msg_properties->rda_status != RS_OPERATE)    &&
       (previous_rda_status == RS_OPERATE))) {
         WR_process_record (block_ptr, byte_cnt, msg_properties, ldm_ver);
         radial_cnt = 0;
         byte_cnt = 0;
         free (block_ptr);
         block_ptr = NULL;
    ...
    Description: Pacakge the data block into a LDM record and write the
          ...
          Input: *buf        - pointer to the data block containing the LDM data
                 ...
                 msg_data    - structure of message data of interest used to process
                               the message
                 ldm_version - the current LDM version number
...
void WR_process_record (char *buf, int data_len, msg_data_t *msg_data, int ldm_version) {
   ...
   sprintf (local_key, "%s%04d%02d%02d%02d%02d%02dV%02d.raw",
            ...
            Vol_tm.tm_hour,Vol_tm.tm_min, Vol_tm.tm_sec, ldm_version);
   ...
   sprintf (ldm_key, "L2-%s/%s/%04d%02d%02d%02d%02d%02d/%d/%d/%c/V%02d/0",
            ...
            Vol_tm.tm_sec, Volume_id, record, key_status, ldm_version);
   ...
   header_len = Populate_uncompressed_data (compressed_buf, compressed_data_len, 
                                            msg_data->radial_status, ldm_version);
...
/********************************************************************************
    ...
    Description: Construct the LDM volume header. This is called when the 
          ...
          Input: ldm_version - the LDM version number being used for data
                               collection for this volume scan
...
static void Construct_volume_hdr (char *vol_hdr, int ldm_version) {
   ...
   sprintf((char *)vol_hdr, "AR2V00%02d.%3.3d", ldm_version, Volume_id);
...
static int Create_and_open_file (char *file_name, int *err) {
   ...
   *err = 0;
   ...
   if( home_dir == NULL )
   {
     ...
     sprintf( working_dir, "%s/command_ldm/", home_dir );
...
static int Populate_uncompressed_data (char *data_buffer, int data_len, 
   ...
   if (data_len == 0) {  /* NOTE: in the original convert_ldm, this was only checked 
      ...
      Construct_volume_hdr (data_buffer, ldm_version);
...
/********************************************************************************
    ...
    Description: Write the data records to the LDM queue

          Input: sequence_num - the record sequence number
                 ldm_key      - the record key
                 len          - the record length
                 *record      - pointer to the record buffer
 ...
 ********************************************************************************/
 ...
 void Write_ldm_data (unsigned sequence_num, const char *ldm_key, unsigned data_length, const char *record) { 

   int hdr_length = sizeof( RPG_LDM_prod_hdr_t );
   ...
   message_length = hdr_length + data_length;

   /* Fill values of RPG LDM product header. */

   RPG_LDM_prod_hdr_init( &RPG_LDM_prod_hdr );
   sprintf( RPG_LDM_prod_hdr.key, "%s", ldm_key );
   /* Clearing flags won't prepend control word. This task does it already. */
   RPG_LDM_prod_hdr.flags = RPG_LDM_PROD_FLAGS_CLEAR;
   RPG_LDM_prod_hdr.feed_type = RPG_LDM_NEXRAD_FEED_TYPE;
   RPG_LDM_prod_hdr.seq_num = sequence_num;
   RPG_LDM_prod_hdr.data_len = data_length;

   /* Allocate space for data buffer and copy into it. */

   if( ( message_buf = malloc( message_length ) ) == NULL )
   {
     LE_send_msg( GL_INFO, "Malloc (%d) failed", message_length );
     return;
   }
   memcpy( &message_buf[0], &RPG_LDM_prod_hdr, hdr_length );
   memcpy( &message_buf[hdr_length], &record[0], data_length );

   /* Write buffer to LB. */

   LE_send_msg (GL_INFO, "Writing LDM message: LDM key = %s size = %d\n", ldm_key, message_length);
   if( ( ret = RPG_LDM_write_to_LB( &message_buf[0] ) ) < 0 )
   {
...
/********************************************************************************
    ...
    Description: Determine the running mode and call the appropriate 
         ...
         Output: LDM_perfmon.total_bytes_c - the total compressed byte count
                 LDM_perfmon.total_bytes_u - the total uncompressed byte count
...
static void Write_record (char *buffer, int compressed_len, int uncompressed_len, 
   ...
   MA_get_level2_modes (&local_mode, &record_mode, &ldm_mode);

   if (local_mode || record_mode) {

      LE_send_msg (GL_INFO,
          "Writing local data: %s, Vol ID: %d, Rec: %d, Size: (C: %d;  U: %d)\n",
          local_key, Volume_id, record_num, compressed_len, uncompressed_len);

      if (record_mode)
         Write_local_data (local_key, buffer, compressed_len, rad_status);
   }

      /* write data to the LDM queue */

#ifndef LEVEL2_TOOL
   if (ldm_mode && !local_mode) {
      Write_ldm_data (Volume_id * 1000 + record_num, ldm_key, compressed_len, buffer);
      LDM_perfmon.total_bytes_c += compressed_len;
      LDM_perfmon.total_bytes_u += uncompressed_len;

         /* Write out LDM Performance Data. */

      ORPGDA_write (ORPGDAT_ARCHIVE_II_INFO, (char *) &LDM_perfmon.total_bytes_c,
                    sizeof(ArchII_perfmon_t), ARCHIVE_II_PERFMON_ID);
   }
#endif
      /* update the LDM statistics counters */

   MA_update_ldm_stats_cntrs (uncompressed_len, compressed_len); 

 ...
 * $Id: RPG_prod_info_to_ldm.c,v 1.6 2012/08/21 13:42:15 ccalvert Exp $
 * $Revision: 1.6 $
 * $State: Exp $
 */
...
static int Lb_id = ORPGDAT_PROD_INFO;
static int Msg_id = PD_LINE_INFO_MSG_ID;
...
static void Write_to_LB()
{
 ...
 if( Is_new_buffer_different() == NO )
  ...
  RPG_LDM_set_LDM_key( &RPG_LDM_prod_hdr, RPG_LDM_PROD_INFO_PROD );

  /* Initialize RPG LDM message header. */
  RPG_LDM_msg_hdr_init( &RPG_LDM_msg_hdr );
  RPG_LDM_msg_hdr.code = RPG_LDM_PROD_INFO_PROD;
  ...
  {
    RPG_LDM_print_product_info_msg( &Data_buffer[0] );
  }

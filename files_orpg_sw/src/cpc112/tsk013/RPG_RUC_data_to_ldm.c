 ...
 * $Id: RPG_RUC_data_to_ldm.c,v 1.6 2012/08/21 13:42:11 ccalvert Exp $
 * $Revision: 1.6 $
 * $State: Exp $
 */
...
static int Msg_id = ORPGDAT_RUC_DATA_MSG_ID;
...
static void Write_to_LB()
{
  ...
  RPG_LDM_set_LDM_key( &RPG_LDM_prod_hdr, RPG_LDM_RUC_DATA_PROD );

  /* Initialize RPG LDM message header. */
  RPG_LDM_msg_hdr_init( &RPG_LDM_msg_hdr );
  RPG_LDM_msg_hdr.code = RPG_LDM_RUC_DATA_PROD;
  ...
  {
    RPG_LDM_print_RUC_data_msg( &Data_buffer[0] );
  }

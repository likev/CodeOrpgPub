 ...
 * $Id: RPG_task_status_to_ldm.c,v 1.6 2012/08/21 13:42:16 ccalvert Exp $
 * $Revision: 1.6 $
 * $State: Exp $
 */
...
static int Lb_id = ORPGDAT_TASK_STATUS;
static int Msg_id = MRPG_PS_MSGID;
...
static void Write_to_LB()
{
  ...
  RPG_LDM_set_LDM_key( &RPG_LDM_prod_hdr, RPG_LDM_TASK_STATUS_PROD );

  /* Initialize RPG LDM message header. */
  RPG_LDM_msg_hdr_init( &RPG_LDM_msg_hdr );
  RPG_LDM_msg_hdr.code = RPG_LDM_TASK_STATUS_PROD;
  ...
  {
    RPG_LDM_print_task_status_msg( &Data_buffer[0], data_length );
  }

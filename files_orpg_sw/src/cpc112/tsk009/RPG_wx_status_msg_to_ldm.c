 ...
 * $Author: ccalvert $
 * $Locker:  $
 * $Date: 2012/08/21 13:42:17 $
 * $Id: RPG_wx_status_msg_to_ldm.c,v 1.6 2012/08/21 13:42:17 ccalvert Exp $
 * $Revision: 1.6 $
 * $State: Exp $
 */
...
static int Msg_id = WX_STATUS_ID;
...
static void Write_to_LB()
{
  ...
  RPG_LDM_set_LDM_key( &RPG_LDM_prod_hdr, RPG_LDM_WX_STATUS_PROD );

  /* Initialize RPG LDM message header. */
  RPG_LDM_msg_hdr_init( &RPG_LDM_msg_hdr );
  RPG_LDM_msg_hdr.code = RPG_LDM_WX_STATUS_PROD;
  ...
  {
    RPG_LDM_print_wx_status_msg( &Data_buffer[0] );
  }

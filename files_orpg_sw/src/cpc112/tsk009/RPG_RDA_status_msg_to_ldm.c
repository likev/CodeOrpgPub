 ...
 * $Author: ccalvert $
 * $Locker:  $
 * $Date: 2012/08/21 13:42:11 $
 * $Id: RPG_RDA_status_msg_to_ldm.c,v 1.6 2012/08/21 13:42:11 ccalvert Exp $
 * $Revision: 1.6 $
 * $State: Exp $
 */
...
static int Msg_id = RDA_STATUS_ID;
...
static void Write_to_LB()
{
  ...
  RPG_LDM_set_LDM_key( &RPG_LDM_prod_hdr, RPG_LDM_RDA_STATUS_PROD );
  
  /* Initialize RPG LDM message header. */
  RPG_LDM_msg_hdr_init( &RPG_LDM_msg_hdr );
  RPG_LDM_msg_hdr.code = RPG_LDM_RDA_STATUS_PROD;
  ...
  {
    RPG_LDM_print_RDA_status_msg( &Data_buffer[0] );
  }

 ...
 * $Author: ccalvert $
 * $Locker:  $
 * $Date: 2012/08/21 13:42:16 $
 * $Id: RPG_vol_status_msg_to_ldm.c,v 1.6 2012/08/21 13:42:16 ccalvert Exp $
 * $Revision: 1.6 $
 * $State: Exp $
 */

/* Reads VOL_STAT_GSM_ID msg from ORPGDAT_GSM_DATA LB */
/* Msg format defined in gen_stat_msg.h as Vol_stat_gsm_t */
...
static int Lb_id = ORPGDAT_GSM_DATA;
static int Msg_id = VOL_STAT_GSM_ID;
...
static void Read_data()
{
  ...
  RPG_LDM_print_debug( "Enter Process_data()" );
...
static void Write_to_LB()
{
  ...
  RPG_LDM_set_LDM_key( &RPG_LDM_prod_hdr, RPG_LDM_VOL_STATUS_PROD );
  
  /* Initialize RPG LDM message header. */
  RPG_LDM_msg_hdr_init( &RPG_LDM_msg_hdr );
  RPG_LDM_msg_hdr.code = RPG_LDM_VOL_STATUS_PROD;
  ...
  {
    RPG_LDM_print_volume_status_msg( &Data_buffer[0] );
  }

 ...
 * $Id: RPG_error_log_to_ldm.c,v 1.5 2012/08/21 13:42:13 ccalvert Exp $
 * $Revision: 1.5 $
 * $State: Exp $
 */
...
static LB_id_t      LB_id = ORPGDAT_ERRLOG;
...
static void Write_to_LB()
{
  ...
  RPG_LDM_set_LDM_key( &RPG_LDM_prod_hdr, RPG_LDM_ERROR_LOG_PROD );

  /* Initialize RPG LDM message header. */
  RPG_LDM_msg_hdr_init( &RPG_LDM_msg_hdr );
  RPG_LDM_msg_hdr.code = RPG_LDM_ERROR_LOG_PROD;
  ...
  {
    RPG_LDM_print_error_log_msg( &product_buffer[prod_hdr_length+msg_hdr_length], Total_size_of_msgs );
  }

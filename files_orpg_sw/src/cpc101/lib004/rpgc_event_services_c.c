 ...
 * $Id: rpgc_event_services_c.c,v 1.15 2012/11/28 15:27:41 steves Exp $
 * $Revision: 1.15 $
 * $State: Exp $
 */
...
static int Task_type = 0;		/* Is this task TASK_EVENT_BASED? */
...
/* Local Functions. */
static int Register_UN( int data_id, LB_id_t msg_id, void (*service_routine)() );
static int Register_event( int event_code, void (*service_routine)(),
                           int queued_parameter, int internal_or_external );
...
/******************************************************************
   Description:
      C/C++ interface for RPG_UN_register.

   Inputs:
      data_id - Data ID of the LB for which the user wishes to be 
                notified when updates occur.
      msg_id - message ID for which the user is interested in updates.
      service_routine - callback routine when notification occurs.

   Returns:
      Returns value from RPG_UN_register call.  
...
/* End of RPGC_UN_register() */
}

/******************************************************************
   Description:
      C/C++ interface for RPG_reg_for_internal_event function.

   Inputs:
      event_code - event ID of the event to be registered.
      service_routine - callback routine called when the event
                        occurs.  
      queued_parameter - parameter passed to the event service
                         routine (callback routine).

   Returns:
      Returns value from RPG_reg_for_internal_event call.  See rpg 
      infrastructure library man page for more details.

*******************************************************************/
int RPGC_reg_for_internal_event( int event_code, 
                                 void (*service_routine)(), 
...
/* End of RPGC_reg_for_internal_event() */
}

/******************************************************************
   Description:
      C/C++ interface for RPG_reg_for_external_event function.
   ...
   Returns:
      Returns value from RPG_reg_for_extrnal_event call.  See 
      rpg infrastructure library man page for more details.

*******************************************************************/
int RPGC_reg_for_external_event( int event_code, 
                                 void (*service_routine)(), 
                                 int queued_parameter ){
...
/* End of RPGC_reg_for_external_event() */
}

/******************************************************************
   Description: 
      This function is used to control the processing of RPG 
      algorithm/product task which are activated based on events 
      rather than the availability of task inputs.  The event queue 
      is examined periodically.  If an event is on the queue, the 
      event service routine associated with the event is called.  
      The queued parameter associated with the registering of this 
      event is passed as an argument.
   ...
   for( i = 0; i < Event_list.num_events; i++ ){
      ...
      if( EVT_WAIT_FOR_EVENT_TIMEOUT == Event_list.code[i] ){
         ...
         if( Evt_wait_for_event == 0 ){
            ...
            RPGC_reg_timer( Event_list.code[i], Timeout_handler );

         }

         /* First cancel the timer if already set, then re-set the
            timer. */
         a3cm41( Event_list.code[i], Event_list.queued_parameter[i], -1, 
                 &status );

         a3cm40( Event_list.code[i], Event_list.queued_parameter[i], -1, 
                 &status );
...
static int Register_event( int event_code, void (*service_routine)(),
   ...
   if( i >= Event_list.num_events ){
      ...
      if( i >= MAX_EVENTS )
         ...
         LE_send_msg( GL_INFO, "Registering Event %d\n", i );
         if( Event_list.internal_or_external[i] == EVT_INTERNAL_EVENT )
            LE_send_msg( GL_INFO, "--->Event Code: %d, INTERNAL, Parameter: %d\n", 
                         Event_list.code[i], Event_list.queued_parameter[i] );
         if( Event_list.internal_or_external[i] == EVT_EXTERNAL_EVENT )
            LE_send_msg( GL_INFO, "--->Event Code: %d, EXTERNAL, Parameter: %d\n", 
                         Event_list.code[i], Event_list.queued_parameter[i] );
...
/********************************************************************
   ...
   Description:
      This function initializes the Event Service module. 

********************************************************************/
void ES_initialize(){

   /* Get the task type ... i.e., is this task EVENT_BASED? */
   Task_type = INIT_task_type ();
   RPGC_log_msg( GL_INFO, "Task Type Registered as %d\n", Task_type );
...
int Service_event_queue(){
   ...
   LB_id_t msg_id;
   int queued_parameter, events_serviced;
   ...
   while( (Event_queue_tail % MAX_QUEUE_SIZE) + 1 != Event_queue_head ){
      ...
      msg_id = Event_queue[Event_queue_head].msg_id;
      ...
      if( Event_queue[Event_queue_head].code == EVT_CFCPROD_REPLAY_PRODUCT_REQUEST )
         ...
         /* If start of volume event, update the product request information. */
         if( (Event_queue[Event_queue_head].code == ORPGEVT_START_OF_VOLUME)
                                          ||
             (Event_queue[Event_queue_head].code == ORPGEVT_START_OF_VOLUME_DATA) )
            PRQ_update_requests( 1 /* new volume */, 1 /* first elevation */,
                                 (int) time(NULL) /* current time */ );
   ...
   for( i = 0; i < Event_list.num_events; i++ ){
      ...
      if( Event_list.internal_or_external[i] == EVT_INTERNAL_EVENT ){
         ...
         switch( Event_list.code[i] ){
            ...
            case EVT_REPLAY_PRODUCT_REQUEST:
            {
            ...
            case EVT_WAIT_FOR_EVENT_TIMEOUT:
            {
               ...
               if( Evt_wait_for_event_timeout ){
                  ...
                  int flag = -1;  /* Indicates internal timer. */
                  int status = 0, count = Event_list.queued_parameter[i];

                  Evt_wait_for_event_timeout = 0;

                  /* Set the timeout value again only if task is not 
                     event based.  If it is event based, we assume this
                     timer is set in RPGC_wait_for_event(). */

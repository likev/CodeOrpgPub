 ...
 * $Author: steves $
 * $Locker:  $
 * $Date: 2012/10/16 17:26:29 $
 * $Id: rpgc_abort_processing_c.c,v 1.24 2012/10/16 17:26:29 steves Exp $
 * $Revision: 1.24 $
 * $State: Exp $
 */
...
static void Set_prod_dep_params( Prod_header *phd, User_array_t *user_array );
static int Write_prod_gen_msg( Prod_gen_msg *phd );
static int Abort_single_output( int datatype, int reason );
static int Check_prod_status( int elev_index, int prod_id, int p_code,
                              int *index, int *status, User_array_t *uarray );
static int Abort_datatype( Prod_header *phd, int olind, int reason );


/* Public Functions. */
/********************************************************************
   Description:
      C/C++ interface for the abort remaining volume scan processing.
      Calls AP_abort_processing with reason PGM_ABORT_REMAIN_SCAN.

   Returns:
      Returns the result of AP_abort_processing.  See man page for
      rpg infrastructure library for more details.
   
********************************************************************/
int RPGC_abort_remaining_volscan( ){

   int reason = PGM_ABORT_REMAIN_SCAN;

   return( AP_abort_processing( reason ) );
   
/* End of RPGC_abort_remaining_volscan( ) */
}

/********************************************************************
   Description:
      C/C++ interface for the abort volume scan processing.
      Calls AP_abort_processing with reason PGM_SCAN_ABORT.

   Returns:
      Returns the result of AP_abort_processing.  

   Note: 
      This has the same behavior as RPGC_abort_because(0).
...
/********************************************************************
   Description:
      C/C++ interface for the abort because processing.
      Calls AP_abort_processing with input argument "reason".

   Input:
      reason - reason for aborting processing.
...
int RPGC_abort_because( int reason ){
   ...
   switch( reason ){
      ...
      case NO_DATA:
      default:
         if( (abort_reason = AP_get_abort_reason()) == 0 )
   ...
   return( AP_abort_processing( abort_reason ) );
   
/* End of RPGC_abort_because( ) */
}

/********************************************************************
   Description:
      C/C++ interface for the abort datatype processing.  Aborts
      data "datatype" with reason "reason".

   Input:
      datatype - datatype which is to be aborted.
      reason - reason for aborting datatype.
...
*/
   ...
   Abort_single_output( datatype, reason );

   return (0);

/* End of RPGC_abort_datatype_because( ) */
}

/********************************************************************
   Description:
      C/C++ interface for the abort dataname processing.  Calls 
      RPGC_abort_datatype_because with input arguments "datatype" 
      and "reason".  "datatype" is returned from 
      OB_get_id_from_name( "dataname" ).

   Input:
      dataname - dataname which is to be aborted.
      reason - reason for aborting processing.

   Returns:
      Returns the result of RPGC_abort_datatype_because().  
...
int RPGC_abort_dataname_because( char *dataname, int reason ){
   ...
   OB_get_id_from_name( dataname, &datatype );
   return( RPGC_abort_datatype_because( datatype, reason ) );

/* End of RPGC_abort_dataname_because( ) */
}
...
***************************************************************************/
int RPGC_abort_request( User_array_t *req, int reason ){
   ...
   pid = ORPGPAT_get_prod_id_from_code( req->ua_prod_code );
   if( pid <= 0 ){
   ...
   /* Fill in the Product request fields. */
   prod_req.pid = pid;
   prod_req.param_1 = req->ua_dep_parm_0;
   prod_req.param_2 = req->ua_dep_parm_1;
   prod_req.param_3 = req->ua_dep_parm_2;
   prod_req.param_4 = req->ua_dep_parm_3;
   prod_req.param_5 = req->ua_dep_parm_4;
   prod_req.param_6 = req->ua_dep_parm_5;
   prod_req.elev_ind = req->ua_elev_index;
...
int RPGC_cleanup_and_abort( int reason ){
   ...
   AP_abort_processing( abort_reason );

   return(0);

/* End of RPGC_cleanup_and_abort( ) */
}

/********************************************************************
   Description:
      Terminates the process calling this function.
...
void RPGC_abort_task( ){
   ...
   PS_task_abort( "Aborting Task\n" );
   return;

/* End of RPGC_abort_task( ) */
}
...
int AP_abort_outputs( int reason ){
   ...
   for( olind = 0; olind < N_outs; olind++ ){
      ...
      if( (Out_list[olind].timing != RADIAL_DATA)
         ...
         /* Abort this datatype. */
         Abort_datatype( &phd, olind, reason );
   ...
   if( reason != PGM_TASK_FAILURE ){
      ...
      if( resume_time == NEW_VOLUME )
         LE_send_msg( GL_INFO, "Task Aborted ... Task Resumes At Start Of NEW VOLUME (%d)\n",
                      NEW_VOLUME );
      else if( resume_time == NEW_ELEVATION )
         LE_send_msg( GL_INFO, "Task Aborted ... Task Resumes At Start Of NEW ELEVATION (%d)\n",
                      NEW_ELEVATION );
      else if( resume_time == NEW_DATA )
         LE_send_msg( GL_INFO, "Task Aborted ... Task Resumes At Start Of NEW DATA (%d)\n",
                      NEW_DATA );
   ...
   for( olind = 0; olind < N_outs; olind++ ){
      ...
      if( Out_list[olind].type == pr->pid ){
         ...
         if( Out_list[olind].timing != RADIAL_DATA &&
	       ...
	       LE_send_msg( GL_INFO, "ABORT MSG: prod %d (vol# %d, wx_mode %d, vcp %d) -> %s (reason %d)\n", 
...
void AP_add_output_to_prod_list( Prod_header *phd ){
   ...
   if( N_prod_list >= Previous_N_prod_list ){
      ...
      Prod_list = (Prod_header *) 
                        MISC_malloc( sizeof(Prod_header)*(N_prod_list+1) );
...
void AP_init_prod_list( unsigned int vol_seq_num ){
   ...
   if( Prod_list == NULL )
      Prod_list = (Prod_header *) MISC_malloc( sizeof(Prod_header)*MAXN_PROD_LIST );
...
/***********************************************************************
    ...
    Description: 
        ...
        algorithm control flag (Alg_control_flg) if input argument has
    ...
    Return:	
        Return value of algorithm control flag (Alg_control_flg).
...
int AP_abort_flag( int flag_value ){
   ...
   if( flag_value == FALSE ){
      ...
      LE_send_msg( GL_INFO, "Clearing the Aborted Flag\n" );
...
/***************************************************************************
   ...
   Inputs:
       ...
       status - status from Check_prod_status or PGS_SCHEDULED
...
int AP_abort_processing ( int reason ){
   ...
   /* Validate the reason for failure.  If invalid reason, terminate. */
   if( (reason > PGM_MAX_GEN_FAILURE) || (reason < PGM_MIN_GEN_FAILURE) )
      PS_task_abort( "AP_abort_processing With Bad Reason (%d)\n", reason );
...
/************************************************************************
   ...
   Inputs:
      ...
      user_array - pointer to structure containing product dependent 
                   parameters.
...
static void Set_prod_dep_params( Prod_header *phd, User_array_t *user_array ){
   ...
   phd->g.req_params[0] = user_array->ua_dep_parm_0;
   phd->g.req_params[1] = user_array->ua_dep_parm_1;
   phd->g.req_params[2] = user_array->ua_dep_parm_2;
   phd->g.req_params[3] = user_array->ua_dep_parm_3;
   phd->g.req_params[4] = user_array->ua_dep_parm_4;
   phd->g.req_params[5] = user_array->ua_dep_parm_5;
...
static int Abort_datatype( Prod_header *phd, int olind, int reason ){
   ...
   int nlist, ret, ind, index, aborted_vol_scan, status;
   int task_type, num_elevation_cuts, aborted, done;
   User_array_t user_array[SLOTS_CAP];
   LB_info list;
   ...
   while(1){
      ...
      index = PRQ_populate_user_array( phd->g.elev_ind, phd->g.prod_id, 
                                       (void *) NULL, aborted_vol_scan, 
                                       user_array, SLOTS_CAP );

      /* If no requests found, check the product status for elevation
         index and volume scan. */
      status = PGS_SCHEDULED;
      if( index == 0 )
         Check_prod_status( phd->g.elev_ind, phd->g.prod_id, 
                               Out_list[olind].int_or_final,
                               &index, &status, user_array );
      ...
      for( ind = 0; ind < index; ind++ ){
         ...
         if( ret != sizeof( Prod_header ) ){
	       ...
	       LE_send_msg ( GL_INFO,
		             ...
		             Out_list[olind].type, phd->g.vol_num, phd->wx_mode, phd->vcp_num,
                             phd->g.elev_ind, Out_list[olind].name, phd->g.len);
#ifdef DEBUG
               ...
               LE_send_msg( GL_INFO, "--->elev_t: %u, vol_t: %u\n", phd->elev_t, phd->g.vol_t );
#endif
         ...
         if( Out_list[olind].event_id != ORPGEVT_NULL_EVENT ){
            ...
            if( ret != 0 )
               ...
               LE_send_msg( GL_INFO, "AP_abort_outputs: Event ID %d Posted.\n",
...
/***************************************************************************
   ...
   Outputs:
       ...
       status - msg ID corresonding to prod_id and elev_index.
      ...
      for( j = 0; j < ps->length; j++ ){
         ...
         if( (entry->prod_id == prod_id)
            ...
            if( (entry->msg_ids[i] == PGS_UNKNOWN)
               ...
               uarray->ua_prod_code = p_code;
               uarray->ua_dep_parm_0 = entry->params[0];
               uarray->ua_dep_parm_1 = entry->params[1];
               uarray->ua_dep_parm_2 = entry->params[2];
               uarray->ua_dep_parm_3 = entry->params[3];
               uarray->ua_dep_parm_4 = entry->params[4];
               uarray->ua_dep_parm_5 = entry->params[5];
               uarray->ua_elev_index = elev_index;
               uarray->ua_req_number = 0;
               uarray->ua_spare = 1;  

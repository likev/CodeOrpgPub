 ...
 * $Author: steves $
 * $Locker:  $
 * $Date: 2014/11/07 21:38:17 $
 * $Id: rpgc_inbuf_c.c,v 1.42 2014/11/07 21:38:17 steves Exp $
 * $Revision: 1.42 $
 * $State: Exp $
 */
...
static int Check_elevation_index = 0;
			/* If this flag is set, we need to check the elevation 
                           ...
                           Added support for tasks having RADIAL_DATA as driving 
                           input but ELEVATION_DATA as non-driving input.  Normally
                           it is sufficient to check elevation time.  However this
                           fails when AVSET is active and terminates the VCP.  In 
                           this case the terminated elevations have the same time
                           as the last elevation in the VCP. */
...
static int* Get_inbuf_any( int *datatype, int *opstat );
...
static void* Get_radial_data_block( void *bufptr, int *type, char *data_name );
static int Find_buffer( void *bufptr, int *ind, int *num );
static int Check_radial_size( int size, char *radial_msg );


/**************************************************************
   Description:
      C/C++ interface for allowing all input products to be
...
int RPGC_reg_inputs( int argc, char *argv[] ){
   ...
   int timing = 0, num_inputs, i, *input_ids = NULL;
   ...
   for( i = 0; i < num_inputs; i++ ){
      ...
      LE_send_msg( GL_INFO, "Registering Input (product ID) %d\n", 
                   input_ids[i]);
      RPGC_in_data( input_ids[i], timing );
         ...
         if( (num_opt_inputs = ORPGPAT_get_num_opt_prods( output_ids[j] )) > 0 ){
            ...
            for( index = 0; index < num_opt_inputs; index++ ){
                ...
                if( opt_prod == input_ids[i] ){
                   ...
                   LE_send_msg( GL_INFO, 
                                "--->Registering Input As Optional (Block Time = 5)\n" ); 
                   RPGC_in_opt( input_ids[i], 5 );
                   done = 1;
                   break;
                  ...
                  LE_send_msg( GL_ERROR, 
                               "ORPGPAT_get_num_opt_prods() Failed For Product %d\n",
                               output_ids[j] );
                  done = 1;
                  break;
...
/**************************************************************
   Description:
      Function for registrating an input.

   Inputs:
      data_type - Product ID of data type to register as input.

   Returns:
      Returns 0 on success, negative number on error.            

   Notes:
      timing value is ignored ..... the library sets the timing
      based on PAT entry.

**************************************************************/
int RPGC_in_data( int data_type, ... ){
   ...
   if( (class_id = ORPGPAT_get_class_id( data_type )) != ORPGPAT_ERROR ){
      ...
      /* Set the class mask if one is defined.  Note:  The ORPGPAT library
         sets the class mask to 0 by default if the class mask is not defined.
         We wish the class mask to be 0xffff if one is not defined. */
      class_mask = ORPGPAT_get_class_mask( data_type );
      if( (class_mask != ORPGPAT_ERROR)
                      &&
          (class_mask != 0) )
   ...
   /* This check supports ingesting radial data as the driving input
      but the non-driving input is elevation data. */
   if( N_inps != DRIVING ){

      if( Inp_list[DRIVING].timing == RADIAL_DATA ){

         if( Inp_list[N_inps].timing == ELEVATION_DATA )
            Check_elevation_index = 1;
...
/*************************************************************
   Description:
      C/C++ interface for the RPG_in_opt routine.

   Inputs:
      data_type - Product ID of data type to register as input.
      block_time - wait time associated with data_type

   Returns:
      Returns value for RPG_in_opt call.  See rpg infrastructure
      library man page for more details.

*************************************************************/
int RPGC_in_opt( int data_type, int block_time ){
...
/*************************************************************
   ...
   Inputs:
      data_name - Product name of data type to register as input.
      block_time - wait time associated with data_name
   ...
   Notes:
      This function also registers the input.
...
int RPGC_in_opt_by_name( char *data_name, int block_time ){
   ...
   IB_get_id_from_name( data_name, &product_id );
   return( RPGC_in_opt( product_id, block_time ) );

/* End of RPGC_in_opt_by_name( ) */
}

/*************************************************************
   Description:
      C/C++ interface for the RPG_get_inbuf routine.

   Inputs:
      reqdata - Product ID of data type requested.
      opstat - pointer to status of the operation.

   Outputs:
      opstat - status of the operation.

   Returns:
      Returns pointer to input buffer on success, or NULL on 
      failure.  On success, opstat is NORMAL, otherwise opstat
      gives the reason for failure. See rpg infrastructure 
      library man page for more details.
...
void* RPGC_get_inbuf( int reqdata, int *opstat ){
   ...
   buff = (int *) IB_get_inbuf( reqdata, &datatype, opstat );
...
/* End of RPGC_get_inbuf( ) */
}

/*************************************************************
   Description:
      C/C++ interface for the RPG_get_inbuf_by_name routine.

   Inputs:
      reqname - Product name of data type requested.
      opstat - pointer to status of the operation.
...
void* RPGC_get_inbuf_by_name( char *reqname, int *opstat ){
   ...
   /* The following function shouldn't fail except possibly 
      because of programming error. */
   IB_get_id_from_name( reqname, &reqdata ); 
   if( reqdata <= 0 ){

      LE_send_msg( GL_INFO, "IB_get_id_from_name Failed\n" );
      *opstat = TERMINATE;
      return( (void *) NULL );

   }

   buff = IB_get_inbuf( reqdata, &datatype, opstat );
...
/* End of RPGC_get_inbuf_by_name( ) */
}

/*************************************************************
   Description:
      C/C++ interface for the RPG_get_inbuf_any routine.

   Inputs:
      datatype - pointer to data type received.
      opstat - pointer status of the operation.

   Outputs:
      datatype - product ID of data type returned.
      opstat - status of the operation.

   Returns:
      Returns pointer to input buffer on success, or NULL on 
      failure.  On success, opstat returns NORMAL, otherwise
      opstat gives the reason for failure.  See rpg 
      infrastructure library man page for more details.

*************************************************************/
void* RPGC_get_inbuf_any( int *datatype, int *opstat ){

   int reqdata = ANY_TYPE;
   int *buff;
...
/* End of RPGC_get_inbuf_any( ) */
}

/*************************************************************
   Description:
      C/C++ interface for the release of input buffer routine.
      Validate the input buffer pointer and if valid, releases
      storage allocated to buffer and removes buffer from 
      IB_inp_list.

   Inputs:
      bufptr - pointer to input buffer to be released.
...
int RPGC_rel_inbuf( void *bufptr ){
   ...
   /* If the buffer pointer is NULL, do nothing. */
   if( bufptr == (void *) NULL )
      return 0;

   /* Find this buffer in the list of acquired buffers. */
   if( Find_buffer( bufptr, &ind, &num ) >= 0 ){
 
      /* Free the space allocated to this buffer. */
...
#ifdef DEBUG
{
         ...
         LE_send_msg( GL_ERROR, "--->Input Buffer @ %p Not Found\n",
                      (int *) bufptr );
         ...
         for( ind = 0; ind < N_inps; ind++ ){
            ...
            for( num = 0; num <= Inp_list[ind].buffer_count; num++ ){
               ...
               LE_send_msg( GL_INFO, "------>Acquired Input Buffer @ %p\n",
                            buffer_addr );
...
/* End of RPGC_rel_inbuf() */
}

/*************************************************************

   Description:
      C/C++ interface to IB_rel_all_inbufs().
   ...
   Returns:
       See IB_rel_all_inbufs() for description of return 
       values.
      
*************************************************************/
int RPGC_rel_all_inbufs(){

   return( IB_rel_all_inbufs() );
...
*************************************************************/
int RPGC_reg_moments( int user_moments ){
...
/*************************************************************
   Description:
      Returns the length, in bytes, of the input buffer 
      specified by "bufptr".
   ...
   Returns:
      If the input buffer is found, returns the length.  
      Otherwise, returns -1.
...
int RPGC_get_inbuf_len( void *bufptr ){
   ...
   if( Find_buffer( bufptr, &ind, &num ) >= 0 ){
      ...
      /* If timing is not radial data, strip off the product header
         length. */
      if( Inp_list[ind].timing != RADIAL_DATA ){

         /* Return the length, in bytes. */
         return( Inp_list[ind].len - sizeof(Prod_header) );

      }
      else
         return( Inp_list[ind].len );
...
/*********************************************************************************
   ...
   Description:
      Get the Basedata Message Type.   Type can be either REFLDATA_TYPE,
      COMBBASE_TYPE or BASEDATA_TYPE.

   Inputs:
      radptr - pointer to radial message.
      check_type - type of interest.
   ...
   Note:
      If buffer pointer does not point to a radial message, the
...
unsigned short RPGC_check_cut_type( void *radptr, unsigned short check_type ){
   ...
   unsigned short type;
   Base_data_header *radhdr = (Base_data_header *) radptr;

   /* Get the type information from the radial header. */
   type = (unsigned short) (radhdr->msg_type & CUT_TYPE_MASK);

   return( type & check_type );

/* End of RPGC_check_basedata_type() */
}
...
/*********************************************************************************
   ...
   Description:
      Get the Radial Message Type.   Type can be either REFLDATA_TYPE,
      COMBBASE_TYPE or BASEDATA_TYPE possibly OR'd with DUAL_POL_TYPE 
      and/or RECOMBINED TYPE. 
...
unsigned short RPGC_check_radial_type( void *radptr, unsigned short check_type ){
   ...
   /* Get the type information from the radial header. */
   type = (unsigned short) (radhdr->msg_type & RADIAL_TYPE_MASK);

   return( type & check_type );

/* End of RPGC_check_radial_type() */
}


/*********************************************************************************
   Description:
      C/C++ interface for retrieving surveillance data from a radial buffer.
      Given pointer to radial buffer, returns pointer to start of surveillance
      data, and values of first good bin and last good bin.

   Inputs:
      bufptr - input buffer pointer.
      first_good_bin - pointer to int 
      last_good_bin - pointer to int.

   Outputs:
      first_good_bin - receives the value of first good surveillance bin.
      last_good_bin - receives the value of last good surveillance bin.

   Returns:
      Pointer to start of surveillance data within a radial, or NULL on failure.

   Notes:
      The input buffer pointer is checked to determine whether it points
      to a radial buffer.  If not a radial buffer with surveillance data, 
      returns NULL. 

*********************************************************************************/
void* RPGC_get_surv_data( void* bufptr, int* first_good_bin, int* last_good_bin ){

   Base_data_header  *radhdr;
   char *buffer_addr = NULL;
   int num, found, ind;
   ...
   */
   if( ind >= N_inps ){
      ...
      LE_send_msg( GL_ERROR, "Input Buffer Pointer Not A Radial\n" );
      return NULL;

   }

   /* 
     Get the starting reflectivity bins and the ending reflectivity bin. 

     NOTE:  The starting bin in the radial header is in reference to a unit
            index array.  Consequently we need to subtract off 1.
   */
   radhdr = (Base_data_header *) bufptr;
   *first_good_bin = (int) radhdr->surv_range - 1;
   *last_good_bin = *first_good_bin + (int) radhdr->n_surv_bins - 1;

   /* 
     Return pointer to start of reflectivity data.
   */
   if( radhdr->n_surv_bins > 0 )
      return ((void *) ((char *) bufptr + radhdr->ref_offset));

   else
      return NULL;

/* End of RPGC_get_surv_data( ) */
}

/****************************************************************************
   Description:
      C/C++ interface for retrieving velocity data from a radial buffer.
      Given pointer to radial buffer, returns pointer to start of velocity
      data, and values of first good bin and last good bin.
   ...
   Outputs:
      first_good_bin - receives the value of first good velocity bin.
      last_good_bin - receives the value of last good velocity bin.

   Returns:
      Pointer to start of velocity data within a radial, or NULL on failure.

   Notes:
      The input buffer pointer is checked to determine whether it points
      to a radial buffer.  If not a radial buffer with velocity data, 
      returns NULL. 
...
void* RPGC_get_vel_data( void* bufptr, int* first_good_bin, int* last_good_bin ){
   ...
   */
   ...
   /* 
     Get the starting velocity bin and the ending velocity bin. 

     NOTE:  The starting bin in the radial header is in reference to a unit
            index array.  Consequently we need to subtract off 1.
   */
   radhdr = (Base_data_header *) bufptr;
   *first_good_bin = (int) radhdr->dop_range - 1;
   *last_good_bin = *first_good_bin + (int) radhdr->n_dop_bins - 1;

   /* 
     Return pointer to start of velocity data.
   */
   if( radhdr->n_dop_bins > 0 )
      return ((void *) ((char *) bufptr + radhdr->vel_offset));

   else
      return NULL;

/* End of RPGC_get_vel_data() */
}

/****************************************************************************
   Description:
      C/C++ interface for retrieving spectrum width data from a radial buffer.
      Given pointer to radial buffer, returns pointer to start of spectrum
      width data, and values of first good bin and last good bin.
   ...
   Outputs:
      first_good_bin - receives the value of first good spectrum width bin.
      last_good_bin - receives the value of last good spectrum width bin.

   Returns:
      Pointer to start of spectrum width data within a radial, or NULL on failure.

   Notes:
      The input buffer pointer is checked to determine whether it points
      to a radial buffer.  If not a radial buffer with spectrum width data, 
      returns NULL. 
...
void* RPGC_get_wid_data( void* bufptr, int* first_good_bin, int* last_good_bin ){
   ...
   */
   ...
   /* 
     Get the starting spectrum width bin and the ending velocity bin. 
   ...
   *last_good_bin = *first_good_bin + (int) radhdr->n_dop_bins - 1;
   ...
   /* 
     Return pointer to start of spectrum width data.
   */
   if( radhdr->n_dop_bins > 0 )
      return ((void *) ((char *) bufptr + radhdr->spw_offset));

   else
      return NULL;

/* End of RPGC_get_wid_data() */
}


/****************************************************************************
   Description:
      C/C++ interface for retrieving radar data a radial buffer.
      Given pointer to radial buffer, returns pointer to the data type
      requested.   

      Information about the data is reported back Generic_moment_t structure. 

   Inputs:
      bufptr - input buffer pointer.
      type - type of data requested.

   Outputs:
      mon - Generic_moment_t structure holding information about data.

   Returns:
      Pointer to start of data within a radial, or NULL if data doesn't 
      exist or on failure.

   Notes:
      The input buffer pointer is checked to determine whether it points
      to a radial buffer.  If not a radial buffer with specific data type
...
void* RPGC_get_radar_data( void* bufptr, int type, Generic_moment_t *mom ){
   ...
   Base_data_header  *radhdr;
   Generic_moment_t *data_block;
   char *buffer_addr = NULL;
   int num, found, ind;

   /* Check to see if "mom" is valid pointer. */
   if( mom == NULL )
      return NULL;
   ...
   /* Search for type of data requested. */
   if( type == ORPGGDR_DANY ){

      char data_name[5];

      memset( data_name, 0, 5 );
      memcpy( data_name, mom->name, 4 );

      data_block = Get_radial_data_block( bufptr, &type, data_name );

   }
   else   
      data_block = Get_radial_data_block( bufptr, &type, NULL );
   ...
   switch( type ){
      ...
      case ORPGGDR_DREF: 
      {
         ...
         /* Fill in items of the "mom" data block header structure. */
         memcpy( mom->name, "DREF", 4 );
         mom->info = 0;
         mom->no_of_gates = radhdr->n_surv_bins;
         mom->first_gate_range = radhdr->range_beg_surv + radhdr->surv_bin_size/2;
         mom->bin_size = radhdr->surv_bin_size;
         mom->tover = 0;
         mom->SNR_threshold = radhdr->surv_snr_thresh;
         mom->data_word_size = 16;
         mom->scale = 2.0;
         mom->offset = 66.0;
      ...
      case ORPGGDR_DVEL:
      {
         ...
         /* Fill in items of the "mom" data block header structure. */
         memcpy( mom->name, "DVEL", 4 );
         mom->info = 0;
         mom->no_of_gates = radhdr->n_dop_bins;
         mom->first_gate_range = radhdr->range_beg_dop + radhdr->dop_bin_size/2;
         mom->bin_size = radhdr->dop_bin_size;
         mom->tover = radhdr->vel_tover;
         mom->SNR_threshold = radhdr->vel_snr_thresh;
         mom->data_word_size = 16;

         if( radhdr->dop_resolution == 1 )
            mom->scale = 2.0;
         else
            mom->scale = 1.0;
      ...
      case ORPGGDR_DSW:
      {
         ...
         /* Fill in items of the "mom" data block header structure. */
         memcpy( mom->name, "DSW", 3 );
         mom->info = 0;
         mom->no_of_gates = radhdr->n_dop_bins;
         mom->first_gate_range = radhdr->range_beg_dop + radhdr->dop_bin_size/2;
         mom->bin_size = radhdr->dop_bin_size;
         mom->tover = radhdr->spw_tover;
         mom->SNR_threshold = radhdr->spw_snr_thresh;
         mom->data_word_size = 16;
         mom->scale = 2.0;
         mom->offset = 129.0;
      ...
      default:
      {
         ...
         memcpy( mom, data_block, sizeof( Generic_moment_t ) );

         /* Return address of data. */
         return( (void *) &data_block->gate.b[0] ); 
...
/* End of RPGC_get_radar_data() */
}

/***************************************************************************

   Description:
      Select a radial data to monitor load.

   Inputs:
      dataname - The data name of the dats to monitor.
...
int RPGC_monitor_input_buffer_load( char *dataname ){
   ...
   /* The following function shouldn't fail except possibly 
      because of programming error. */
   IB_get_id_from_name( dataname, &data_id ); 
   if( data_id <= 0 ){
   ...
   /* Check that this "data_id" is valid, this "data_id" is also the driving 
      input, and the input's timing is RADIAL_DATA. */
   ...
   LE_send_msg( GL_INFO, "Monitoring Input Buffer Load for Product ID: %d\n",
                data_id );
   return 0;

/* End of RPGC_monitor_input_buffer_load() */
}
...
/***********************************************************************
    ...
    Return:	
        The address of the input buffer on success, or NULL on failure.
...
void* IB_get_inbuf( int reqdata, int *datatype, int *opstat ){
    ...
    *opstat = NORMAL;
	...
	return (Get_inbuf_any (datatype, opstat));
    ...
    *datatype = reqdata;
    ...
    if ( *opstat == NORMAL ){
       ...
       /* We can return to caller at this point.  For non-radial inputs, we need to
          account for the ORPG product header. */
       if( Inp_list[ind].timing != RADIAL_DATA )
          bufptr = (char *) Inp_list[ind].buf[buffer_ind] + sizeof(Prod_header);

       else
          bufptr = (char *) Inp_list[ind].buf[buffer_ind];
       ...
       /* Abort processing for this volume scan.
         ...
         NOTE:  If product header pointer is not NULL, then an abort condition
                ...
                have no header.  In the first case, AP_abort_processing must be 
...
int IB_read_driving_input ( int *buffer_ind ){
   ...
   /* Initialize return value to NORMAL and buffer_ind to invalid value. */
   *buffer_ind = -1;
   ...
   if( Inp_list[DRIVING].timing == RADIAL_DATA ){
	 ...
	 if (Use_direct){
	    ...
	    if( size == LB_BAD_ACCESS || size == RSS_NOT_SUPPORTED) {
               ...
               Use_direct = 0;
               bd_buf = NULL;
               LE_send_msg( GL_INFO, "Direct Access Not Supported\n" );
	 ...
	 if( (size > 0) && (Check_radial_size( size, bd_buf ) == 0) ){
	    ...
	    continue;
         ...
         /* Data has expired from LB.  For realtime data, skip to the first
            unread message, then try again.  Otherwise check if we should wait 
            for the data to become available.  If not, invalidate the buffer 
            index and return NO_DATA.  */
	 else if( size == LB_EXPIRED ){

            LE_send_msg( GL_ERROR, "Read Driving Input .... LB_EXPIRED\n" );
            ...
            if( Input_stream == PGM_REALTIME_STREAM ){
               ...
               /* Skip to the first unread message in the LB. */
               ORPGDA_seek( Inp_list[DRIVING].data_id, 0, LB_FIRST, NULL );
...
static int Read_other_input( int ind, int *buffer_ind ){
   ...
   *buffer_ind = -1;
   ...
   LE_send_msg( GL_INFO, "------>Driving Elevation Index: %d\n", driving_elev_index );
#endif
   ...
   while (1) {
      ...
      int other_nlist, other_elev_index, is_abort_msg, i;
      time_t other_time = 0;
      ...
      for( i = other_nlist - 1; i >= 0; i-- ){
         ...
         if( idata->timing != TIME_DATA ){
            ...
            /* This test will fail if the product is not currently 
               ...
               If not an elevation product, the time must match.  */
...
/********************************************************************
   ...
   Description: 
    ...
    Outputs:	 datatype - returns the data type.
		 opstat - status as defined in a309.h.
...
static int* Get_inbuf_any (int *datatype, int *opstat){
   ...
   *opstat = NO_DATA;
   ...
   if( Inp_list[ind].vol_num > last_vol_num_processed ){
      ...
      LE_send_msg( GL_INFO, "Processing Data For Volume Scan %d\n", 
   ...
   *datatype = Inp_list[ind].type;
   ...
   if( WA_data_filtering( Inp_list + ind, buffer_ind ) != DATA_NEEDED ){
      ...
      LE_send_msg( GL_INFO, "Data Type %d Not Needed By This Application\n", 
                   *datatype );
      IB_release_input_buffer( ind, buffer_ind );
      return( NULL );
   ...
   if( *opstat == NORMAL ){            
      ...
      LE_send_msg( GL_INFO, "Product %d For Volume %d Returned To Application\n",
                   phd->g.prod_id, phd->g.vol_num );

      bufptr = Inp_list[ind].buf[buffer_ind] + sizeof(Prod_header);
      return( (void *) bufptr );
...
/********************************************************************
    ...
    Outputs:    vol_seq_num - volume sequence number
                ...
                is_abort_msg - if set, product is an abort message
...
static int Read_product( char **buf, LB_id_t msg_id, int ind, 
   ...
   *return_status = NORMAL;
   ...
   /* Check if the product was compressed.  If compressed, need to decompress
      the product. */
   if( ((type = ORPGPAT_get_compression_type( phd->g.prod_id )) > 0) 
                               &&
       (idata->timing != RADIAL_DATA) ){

      dest = CP_decompress_product( *buf, &size ); 
...
#ifdef DEBUG
      LE_send_msg( GL_INFO, "... Check_for_abort_message ...\n" );
...
static int Check_moments( int ind, Base_data_header *bdh ){
   ...
   RPGC_what_moments( bdh, &ref_flag, &vel_flag, &wid_flag );
...
/***************************************************************************
   ...
   Description:
      Given a pointer to a radial and type requested, determine if the
      type exists in the radial.   If exists, return a pointer to where
      the data is located.   Otherwise return NULL.

   Inputs:
      bufptr - Radial message.
      data_type - pointer type of requested data.
      data_name - user-supplied name if *type == ORPGGDR_DANY.

   Outputs:
      data_type = data type returned if on input *data_type == ORPGGDR_DANY.

   Returns:
      Pointer to data of interest.

***************************************************************************/
static void* Get_radial_data_block( void *bufptr, int *data_type, char *data_name ){

   Base_data_header *radhdr = (Base_data_header *) bufptr;

   int type = *data_type;
   
   /* If asking for any type, try and find match on moment types. */
   if( type == ORPGGDR_DANY ){

       if( strstr( data_name, "DREF" ) )
          *data_type = ORPGGDR_DREF;

       else if( strstr( data_name, "DVEL" ) )
          *data_type = ORPGGDR_DVEL;

       else if( strstr( data_name, "DSW" ) )
          *data_type = ORPGGDR_DSW;
   ...
   switch( type ){
      ...
      default:
      {
         ...
         for( i = 0; i < radhdr->no_moments; i++ ){

            char str_type[5];
            Generic_any_t *data_block = 
                          (Generic_any_t *) (((char *) bufptr) + radhdr->offsets[i]);

            /* Convert the name to a string so we can do string compares. */
            memset( str_type, 0, 5 );
            memcpy( str_type, data_block->name, 4 );

            if( (type == RPGC_DRF2) && (strstr( str_type, "DRF2" ) != NULL) )
               return( (char *) data_block );

            else if( (type == ORPGGDR_DRHO) && (strstr( str_type, "DRHO" ) != NULL) )
               return( (char *) data_block );

            else if( (type == ORPGGDR_DPHI) && (strstr( str_type, "DPHI" ) != NULL) )
               return( (char *) data_block );

            else if( (type == ORPGGDR_DZDR) && (strstr( str_type, "DZDR" ) != NULL) )
               return( (char *) data_block );

            else if( (type == ORPGGDR_DSNR) && (strstr( str_type, "DSNR" ) != NULL) )
               return( (char *) data_block );

            else if( (type == ORPGGDR_DRFR) && (strstr( str_type, "DRFR" ) != NULL) )
               return( (char *) data_block );

            else if( (type == ORPGGDR_DSMZ) && (strstr( str_type, "DSMZ" ) != NULL) )
               return( (char *) data_block );

            else if( (type == ORPGGDR_DSMV) && (strstr( str_type, "DSMV" ) != NULL) )
               return( (char *) data_block );

            else if( (type == ORPGGDR_DKDP) && (strstr( str_type, "DKDP" ) != NULL) )
               return( (char *) data_block );

            else if( (type == ORPGGDR_DSDP) && (strstr( str_type, "DSDP" ) != NULL) )
               return( (char *) data_block );

            else if( (type == ORPGGDR_DSDZ) && (strstr( str_type, "DSDZ" ) != NULL) )
               return( (char *) data_block );

            else if( (type == ORPGGDR_DANY) && (strstr( str_type, data_name ) != NULL) )
               return( (char *) data_block );
...
/* End of Get_radial_data_block( ) */
}

/***************************************************************************

   Description:
      Find the input buffer specified by "bufptr" and return the "ind" and
      "num"
   ...
   Outputs:
      ind - index into Inp_list corresponding to buffer
      num - number of buffer in buffer list

   Returns:
      0 if buffer found, or -1 is buffer not found.
...
static int Find_buffer( void *bufptr, int *ind, int *num ){
   ...
   int   loc_ind = -1, loc_num = -1, found;
   char *buffer_addr = NULL;
  
   /* Search the input list for match on buffer pointer. */
   found = 0;
   for( loc_ind = 0; loc_ind < N_inps; loc_ind++ ){
   
      for( loc_num = 0; loc_num <= Inp_list[loc_ind].buffer_count; loc_num++ ){
   
         /* For non-radial type inputs, must account for product header.
            The buffer pointer returned to the application is the malloced
            address plus the size of the product header. */
         buffer_addr = Inp_list[loc_ind].buf[loc_num];
         if( Inp_list[loc_ind].timing != RADIAL_DATA )
            buffer_addr += sizeof(Prod_header);
   ...
   if( loc_ind >= N_inps ){
      ...
      LE_send_msg( GL_ERROR, "Releasing Unknown Input Buffer Pointer (%p)\n",
                     bufptr );
      return(-1);

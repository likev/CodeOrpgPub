 ...
 * $Id: rpgc_prod_compress_c.c,v 1.6 2008/12/04 14:52:09 steves Exp $ 
 * $Revision: 1.6 $ 
 * $State: Exp $ 
 */ 
#include <rpgc.h>
...
/***************************************************************
   Description:
      C/C++ interface routine for compressing a product.  

   Inputs:
      bufptr - pointer to algorithm product buffer.
      method - method of product compression.
...
int RPGC_compress_product( void *bufptr, int method ){
   ...
   /* Call the RPG library routine for product compression. */
   int int_or_final;
...
/* RPGC_compress_product() */
}

/***************************************************************
   Description:
      C/C++ interface routine for decompressing a product.

   Inputs:
      src - pointer to algorithm output buffer.

   Returns:
      Pointer to decompressed product on success, NULL on error.

***************************************************************/
void* RPGC_decompress_product( void *src ){

   int dest_len = 0;

   /* Call the routine to do the decompression. */
   return( (void *) CP_decompress_product( src, &dest_len ) );

/* RPGC_decompress_product() */
}

/***************************************************************
   Description:
      Decompresses the product stored at src.  
   ...
   Returns: 
      Pointer to the decompressed product on success, NULL on
      error.
...
void* CP_decompress_product( void *src, int *size ){
   ...
   int int_or_final;
   Prod_header *phd = (Prod_header *) src;
   ...
   if( (int_or_final = ORPGPAT_get_code( phd->g.prod_id )) < 0 ){
      ...
      LE_send_msg( GL_ERROR, "ORPGPAT_get_code(%d) Return Error: %d\n",
                   phd->g.prod_id, int_or_final );
   ...
   if( int_or_final == INT_PROD )
      CP_decompress_intermediate_product( src, &dest, size );

   else
      CP_decompress_final_product( src, &dest, size );

   return( (void *) dest );

/* CP_decompress_product() */
}
...
static int CP_compress_intermediate_product( void *bufptr, int method ){
   ...
   int total_len, src_len, dest_len;
   ...
   switch( method ){
      ...
      case COMPRESSION_BZIP2:
         ...
         /* Call BZIP2 compression. */
         dest_len = MISC_compress( MISC_BZIP2, src, src_len, dest, dest_len );
         break;
      ...
      case COMPRESSION_ZLIB:
         ...
         /* Call ZLIB (GZIP) compression. */
         dest_len = MISC_compress( MISC_GZIP, src, src_len, dest, dest_len );
         break;
...
static int CP_compress_final_product( void *bufptr, int method ){
   ...
   int length, total_len, src_len, dest_len;
   ...
   if( dest_len >= 0 ){
      ...
      RPGC_set_product_int( (void *) &phd->msg_len, length ); 

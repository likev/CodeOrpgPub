 ...
 * $Author: steves $
 * $Locker:  $
 * $Date: 2011/07/27 15:29:20 $
 * $Id: rpgc_itc_c.c,v 1.7 2011/07/27 15:29:20 steves Exp $
 * $Revision: 1.7 $
 * $State: Exp $
 */
...
/**************************************************************
   Description:
      C/C++ interface for registering an input ITC.

   Inputs:
      itc_id - ITC id.
      first - address of start of ITC data.
      size - size of the ITC data, in bytes.
...
**************************************************************/
int RPGC_itc_in( int itc_id, void *first, unsigned int size,
                 int sync_prd, ... ){
...
/**************************************************************
   Description:
      C/C++ interface for registering an output ITC.
...
**************************************************************/
int RPGC_itc_out( int itc_id, void *first, unsigned int size,
                 int sync_prd, ... ){
...
/*************************************************************
   Description:
      C/C++ interface for registering a callback for the ITC
      "itc_id".
...
int RPGC_itc_callback( int itc_id, int (*func)() ){
   ...
   if (!itc_registered) {
      ...
      PS_task_abort ("unregistered ITC specified in RPGC_itc_callback\n");
      return (0);
...
/* End of RPGC_itc_callback( ) */
}

/*************************************************************
   Description:
      C/C++ interface for reading a registered ITC.
...
/* End of RPGC_itc_read( ) */
}

/*************************************************************
   Description:
      C/C++ interface for writing a registered ITC.
...
int RPGC_itc_write( int itc_id, int *status ){
   ...
   *status = NORMAL;
   ...
   for (i = 0; i < N_itcs; i++) {
      ...
      /* If ITC match and access mode is write, break out of loop. */
...
void ITC_write_all (int out_prod){
    ...
    for (i = 0; i < N_itcs; i++) {
        ...
        if( (Itc [i].sync_prd == out_prod) && (Itc [i].access == ITC_WRITE) ){
            ...
            /* Write the ITC. */
            RPGC_itc_write( Itc [i].id, &status );
            if (status != NORMAL)

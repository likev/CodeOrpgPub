 ...
 * $Id: rpgc_outbuf_c.c,v 1.35 2013/10/24 14:24:44 steves Exp $
 * $Revision: 1.35 $
 * $State: Exp $
 */
...
static int Force_use_suppl_scans = 0;		/* Use supplemental scans ... force use if
...
static int Find_buffer_in_list( void *user_bufptr, int disposition,
                                int *buf_ind, int *out_ind );
...
/**************************************************************
   Description:
      All outputs are registered using a single function call.
      Uses information from the task's Task Attribute Table
      entry to determine the task's outputs.
...
int RPGC_reg_outputs( int argc, char *argv[] ){
   ...
   for( i = 0; i < num_outputs; i++ ){
      ...
      LE_send_msg( GL_INFO, "Registering Output (product ID) %d\n",
                   output_ids[i]);
      RPGC_out_data( output_ids[i], timing, product_code );
...
/* End of RPGC_reg_outputs() */
}

/**************************************************************
   Description:
      C/C++ interface for registering an output product.

   Inputs:
      data_type - Product ID of data to be registered as 
                  output.
...
**************************************************************/
int RPGC_out_data( int data_type, ... ){
...
/* End of RPGC_out_data() */
}

/*************************************************************
   Description:
      C/C++ interface for registering an output by name 
      associated with an event.

   Inputs:
      data_name - Product name of data to be registered as
                  output.
      event_id - event ID associated with output type.

   Returns:
      Returns product_id on success, -1 on error.

*************************************************************/
int RPGC_out_data_by_name_wevent( char *data_name, en_t event_id ){
   ...
   if( prod_id > 0 ){
      ...
      if( RPGC_out_data_wevent( prod_id, event_id ) < 0 )
         prod_id = -1;

   }

   if( prod_id <= 0 )
      LE_send_msg( GL_ERROR, "Registering Data Name %s With Event %d Failed\n",
                   data_name, event_id );

   return( prod_id );

/* End of RPGC_out_data_by_name_wevent( ) */
}

/*************************************************************
   Description:
      C/C++ interface for registering an output associated
      with an event.
...
*************************************************************/
int RPGC_out_data_wevent( int data_type, en_t event_id, ... ){
   ...
   LE_send_msg( GL_ERROR, "Registering Data ID %d With Event %d Failed\n",
                data_type, event_id );

   return (-1);
   
/* End of RPGC_out_data_wevent( ) */
}

/********************************************************************

   Description:
      Allows a previously allocated output buffer to be reallocated.

   Inputs:
      bufsiz - size in bytes of output buffer.

   Outputs:
      opstat - status of the operation.

   Returns:
      Returns pointer to output buffer on succes, or NULL on failure.

   Notes:
      If allocation fails for any reason, the original output buffer
      is left intact.

*********************************************************************/   
void* RPGC_realloc_outbuf( void *user_bufptr, int bufsize, int *opstat ){

    static char *bd_buf = NULL; /* base data buffer which is never freed */
    static int bd_buf_size = 0; /* size of bd_buf */
    char *cpt = NULL;
    int *bufptr, error, n_tests, ind, olind;
    int size, bufsize_ints = 0;
    Prod_header *phd;


   /* Verify that the buffer size is valid. */
   if( (bufsize < 0) || (bufsize > 8000000) ){

      LE_send_msg( GL_ERROR, "Invalid Output Buffer Size\n" );
      *opstat = NO_MEM;
      return( (void *) NULL );

   }

   /* Find the buffer in the lists "Buffers" and "Out_list" */
   error = Find_buffer_in_list( user_bufptr, FORWARD, &ind, &olind );
   if( error < 0 ){

      LE_send_msg( GL_ERROR, "Original Output Buffer Not Found\n" );
      *opstat = NO_MEM;
      return( (void *) NULL );

   }

   /* Convert buffer size to number of ints. */
   bufsize_ints = ((bufsize+sizeof(int)-1) / sizeof( int )) + 1;
   size = bufsize_ints * 4 + sizeof (Prod_header);
    ...
    /* Output buffer acquired successfully.  Copy contents of orginal buffer
       to the new buffer. */
    memcpy( cpt, Buffers[ind].bpt, Out_list[olind].len );
   
    /* Free the original buffer. */
    free( Buffers[ind].bpt );

    /* Set length of the buffer in Out_list. */
    Out_list[olind].len = bufsize_ints * 4 + sizeof (Prod_header);
    ...
    /* Calculate index into "mem" buffer.  This is necessary to support legacy
       algorithms. */
    bufptr = (int *)(cpt + sizeof (Prod_header));

    /* Register the buffer. */
    Buffers[ind].size = bufsize_ints;
    Buffers[ind].bpt = cpt;
        
    /* Return status of operation as NORMAL. */
    *opstat = NORMAL;

   /* Return address of output buffer. */
   return( (void *) bufptr );
      
} /* End of RPGC_realloc_outbuf() */

/*************************************************************
   Description:
      C/C++ interface for getting an output buffer.  The 
      requested buffer size (in bytes) is validated, and if
      an appropriate size, a buffer is allocated.

   Inputs:
      dataname - product name of requested output buffer type.
      bufsiz - size in bytes of output buffer.
   ...
   Returns:
      Returns pointer to output buffer on success, or NULL 
      on failure.

   Notes:
      If unsuccessful, a NULL pointer is returned and "opstat" 
      gives the reason it was unsuccessful.  On success, a
      valid pointer is returned and "opstat" will return
      NORMAL.

      See rpg infrstructure library man page for more details.

*************************************************************/
void* RPGC_get_outbuf_by_name( char *data_name, int bufsiz, 
                               int *opstat ){

   int datatype = 0;

   /* The following shouldn't fail except possibly because 
      of programming error. */
   OB_get_id_from_name( data_name, &datatype );
   if( datatype <= 0 ){

      LE_send_msg( GL_INFO, "OB_get_id_from_name Failed: %d\n", datatype );
      *opstat = NO_DATA;
      return( (void *) NULL );

   }

   return( RPGC_get_outbuf( datatype, bufsiz, opstat ) );

/* End of RPGC_get_outbuf_by_name() */
}
...
/*************************************************************
   ...
   Inputs:
      datatype - product ID of requested output buffer type.
      bufsiz - size in bytes of output buffer.
      opstat - pointer to status of the operation.
...
void* RPGC_get_outbuf( int datatype, int bufsiz, int *opstat ){
   ...
   static User_array_t user_array;

   /* Verify that the buffer size is valid. */
   if( bufsiz < 0 || bufsiz > 8000000 ){
   ...
   /* Initialize the status of operation to NORMAL. */
   *opstat = NORMAL;

   if( datatype != SCRATCH ){

      /* Check if this datatype is requested for generation. */
      memset( &user_array, 0, sizeof(User_array_t) );
      PRQ_check_data( datatype, (void *) &user_array, opstat ); 
   ...
   if( *opstat == NORMAL ){
      ...
      /* Convert buffer size to number of ints. */
      bufsize_ints = ((bufsiz+sizeof(int)-1) / sizeof( int ));

      /* Acquire the output buffer. */
      bufptr = OB_get_outbuf( datatype, bufsize_ints, &olind, opstat );

   }

   /* Non-normal status always returns NULL pointer. */
   if( *opstat != NORMAL )
      return ( (void *) NULL );

   /* Copy the request information "user_array" to output buffer
      product header. */
   if( datatype != SCRATCH )
      OB_tag_outbuf_wreq( bufptr, (void *) &user_array, olind );

   /* Return address of output buffer. */
   return( (void *) bufptr );

/* End of RPGC_get_outbuf() */
}
...
/*************************************************************
   ...
   Inputs:
      datatype - product ID of requested output buffer type.
      bufsiz - size in bytes of output buffer.
      user_array- User_array_t structure containing product request
                  data.
      opstat - pointer to status of the operation.
...
void* RPGC_get_outbuf_for_req( int datatype, int bufsiz, 
   ...
   *opstat = NORMAL;
   ...
   /* Check if datatype is to be generated.  "opstat" will return
      NOT_REQD if not required, otherwise NORMAL is returned. */
   if( datatype != SCRATCH )
      PRQ_check_req( datatype, (void *) user_array, opstat );

   if( *opstat == NORMAL ){

      /* Convert buffer size to number of ints. */
      bufsize_ints = (bufsiz / sizeof( int )) + 1;
      bufptr = OB_get_outbuf( datatype, bufsize_ints, &olind, opstat );
   ...
   /* Tag the output buffer with product request data. */
   OB_tag_outbuf_wreq( bufptr, (void *) user_array, olind );

   /* Return address of output buffer. */
   return( (void *) bufptr );

/* End of RPGC_get_outbuf_for_req() */
}
...
*************************************************************/
void* RPGC_get_outbuf_by_name_for_req( char *data_name, int bufsiz,
   ...
   if( datatype <= 0 ){
      ...
      LE_send_msg( GL_INFO, "OB_get_id_from_name Failed\n" );
      *opstat = NO_DATA;
      return( (void *) NULL );

   }

   return( RPGC_get_outbuf_for_req( datatype, bufsiz, user_array,
                                    opstat ) );

/* End of RPGC_get_outbuf_by_name_for_req() */
}
...
/********************************************************************
   ...
   Returns:
      Either value pcode if data_name refers to the final
...
int RPGC_get_code_from_name( char *data_name ){
   ...
   return( ORPGPAT_get_code( prod_id ) );

/* End of RPGC_get_code_from_name() */
}

/********************************************************************

   Description:
      Returns the product ID given the data name.  Accepts either
      an output name or an input name.

   Inputs:
      data_name - name of the input or output.

   Returns:
      Returns product ID.  If not found, returns -1.

   Notes:
      This function must be called after RPGC_task_init().
...
int RPGC_get_id_from_name( char *data_name ){
   ...
   /* Check the outputs first.  If returns -1, then check the inputs. */
   OB_get_id_from_name( data_name, &prod_id );

   if( prod_id == - 1)
      IB_get_id_from_name( data_name, &prod_id );

   return( prod_id );

/* End of RPGC_get_id_from_name() */
}
...
/********************************************************************
   ...
   Returns:
      Either value pcode if prod_id refers to the final
...
/* End of RPGC_get_code_from_id() */
}

/*************************************************************
   Description:
      C/C++ interface for releasing an output buffer.  First
      check if the buffer was first acquired.  If yes, the
      buffer space is released and removed from Out_list.

   Inputs:
      user_bufptr - pointer to output buffer.
      disposition - either FORWARD or DESTROY.
      ... - extended argument.  Contains the size of the 
            buffer.

   Returns:
     Returns 0 if all is normal, otherwise returns:

        RPGC_BUF_NOT_FOUND - user buffer not registered and
                             disposition is DESTROY

   Notes:
      If buffer pointer of buffer to be released is invalid,
      the process terminates is disposition is FORWARD.
      
*************************************************************/
int RPGC_rel_outbuf( void *user_bufptr, int datdis, ... ){

   int ind, olind, error, size;
   ...
   /* Find the buffer in the lists "Buffers" and "Out_list" */
   error = Find_buffer_in_list( user_bufptr, disposition, &ind, &olind );

   if( error < 0 ) 
      return( error );

   /* If there is an extended argument, process it here.  If the buffer
      is going to be destroy, don't bother. */
   if( extended_args && (disposition != DESTROY) ){
      ...
      if( Out_list[olind].int_or_final == INT_PROD ){
         ...
         /* For intermediate products which are not radial data. */
         if( Out_list[olind].timing != RADIAL_DATA ){

            phd = (Prod_header *) Buffers[ind].bpt;
            ...
            if( (size < 0) || ((size + sizeof(Prod_header)) > phd->g.len) )
               LE_send_msg( GL_ERROR, 
                            "Improbable Size: < 0 or > %d (%d) in RPGC_rel_outbuf\n",
                            phd->g.len, size + sizeof(Prod_header) );
         ...
         /* For intermediate products which are radial data. */
         else{

            /* If the size specified is an improbable length, don't change the
               size.  Release the full product. */ 
            if( (size < 0) || ((size + sizeof(Prod_header)) > Out_list[olind].len) )
               LE_send_msg( GL_ERROR, 
                            "Improbable Size: < 0 or > %d (%d) in RPGC_rel_outbuf\n",
                            Out_list[olind].len, size + sizeof(Prod_header) );
   ...
   /* We set the product generation time here. */
   if( (Out_list[olind].int_or_final != INT_PROD) 
                        &&
           (disposition != DESTROY) ){
      ...
      time_t cur_time = 0;
      Graphic_product *descrip_block = (Graphic_product *) user_bufptr;
      ...
      if( (Out_list[olind].int_or_final >= ORPGDBM_MIN_PCODE)
         ...
         RPGCS_unix_time_to_ymdhms( cur_time, &yr, &mo, &day,
                                       &hr, &min, &sec );
         ...
         /* Write out time stamp .... helps with debugging. */
...
/* End of RPGC_rel_outbuf() */
}

/****************************************************************************

   Description:
      Releases all output buffers with disposition "datadis".

   Input:
      datadis - data disposition

   Outputs:

   Returns:
      See OB_rel_all_outbufs() for description of return values.

****************************************************************************/
int RPGC_rel_all_outbufs( int datadis ){

   /* Validate the data disposition.  Must be either FORWARD or DESTROY.  If
      neither, set it to DESTROY. */
   if( (datadis != FORWARD) && (datadis != DESTROY) )
      datadis = DESTROY;

   return( OB_rel_all_outbufs( &datadis ) );

}
 
/*************************************************************
   Description:
      C/C++ interface for RPGC_check_data_by_name().  The
      function RPGC_check_data() is defined in RPGC_prod_request.c.
      The "by_name" function is defined here because the
      translate function from name to ID is OB_get_id_from_name().

      Typically this function is called for outputs.  It has 
      been modified to work with inputs.

   Inputs:
      dataname - data name of input/output buffer name.
   ...
   Notes:
      See RPGC_check_data() in rpgc_prod_request_c.c for more
      details.
...
int RPGC_check_data_by_name( char *data_name ){
   ...
   if( datatype <= 0 ){
      ...
      if( datatype  < 0 )
         IB_get_id_from_name( data_name, &datatype );
   ...
   if( datatype < 0 ){
      ...
      LE_send_msg( GL_INFO, "OB/IB_get_id_from_name Failed: %d\n",
                   datatype );
      return( NOT_REQD );

   }

   return( RPGC_check_data( datatype ) );

/* End of RPGC_check_data_by_name() */
}


/***************************************************************************
   Description:
      Find the buffer in the "Buffers" and "Out_list" lists.
   ...
   Outputs:
      buf_ind - receives index into "Buffers"
      out_ind - receives index into "Out_list"
...
static int Find_buffer_in_list( void *user_bufptr, int disposition,
   ...
   /* Account for the ORPG product header at start of buffer. */
   bufptr =  (char *) user_bufptr - sizeof(Prod_header);

   /* Search for the record. */
   for (ind = 0; ind < N_bufs; ind++) {
   
      if( (Buffers[ind].bpt == bufptr)
                     ||
          (Buffers[ind].bpt == (char *) user_bufptr) )
         break;
   ...
   /* If buffer not found in buffer list (i.e., attempting to release
      a buffer that was never acquired), then abort or return error
      depending on disposition. */
   if( ind >= N_bufs ){

      if( disposition == DESTROY ){

         LE_send_msg( GL_ERROR, "Attempting Release of Output Buffer Not Found\n");
         return( RPGC_BUF_NOT_FOUND );

      }
      else
         PS_task_abort ("Attempting Release of Output Buffer Not Found\n");

   }
   
   if( Buffers[ind]. type == SCRATCH )
      olind = N_outs + 1;

   else{

      /* Find the index in Out_list. */
      ...
      /* If buffer not found in output list abort or return error depending 
         on disposition. */
      if( olind >= N_outs ){
         ...
         if( disposition == DESTROY ){
            ...
            LE_send_msg( GL_ERROR, 
                         "Attempting Release of Output Buffer Not Registered\n" );
            return( RPGC_BUF_NOT_FOUND );

         }
         else
            PS_task_abort( "Attempting Release of Output Buffer Not Registered\n" );
...
/***********************************************************************
	...
	opstat - status as defined in iobuf.h: NORMAL, NOT_REQD, which 
        ...
        olind - index of buffer in Out_list.

    Return:	

        Pointer to the output buffer on success, NULL on error.
...
int* OB_get_outbuf( int dattyp, int bufsiz, int *olind, int *opstat ){
    ...
    char *cpt;
    Prod_header *phd;
    int ind, size, n_tests, i;			
    int *bufptr = NULL;

    /* Initialize olind to error value. */
    *olind = -1;
	...
	for( ind = 0; ind < N_outs; ind++ ){
	    ...
	    if (dattyp == Out_list[ind].type) {

                 *olind = ind;

                /* If not requested and not DEMAND_DATA, data is not
                   required. */
		if (!(Out_list[ind].requested)
                              &&
                     (Out_list[ind].timing != DEMAND_DATA)) {
	...
	Out_list[ind].len = size;
	if (Out_list[ind].timing == RADIAL_DATA) {
	...
	else if (ind >= 0 && Out_list[ind].timing == RADIAL_DATA){
    ...
    /* Save the address of the output buffer.   This is the return value. 
       Will be adjusted for the size of the Prod_header if not radial
       data. */
    bufptr = (int *) cpt;

    /* If the output buffer is not radial data, clear out the Prod_header
       and initialize some fields. */
    if( (ind >= 0) && (Out_list[ind].timing != RADIAL_DATA) ){

       /* Output buffer acquired successfully.  Initialize orpg product header. */
       phd = (Prod_header *) cpt;
       memset( phd, -1, sizeof(Prod_header) );

       /* The buffer pointer returned is the address where the application can
          write product data. */
       bufptr = (int *)(cpt + sizeof (Prod_header));

       /* Initialize the compression method and size fields.  All the other
          fields will be set upon release of the output buffer. */ 
       phd->compr_method = COMPRESSION_NONE;
       phd->orig_size = phd->g.len = size;
    ...
    if( ORPGPAT_get_code( dattyp ) > 0 )
       memset( (void *) bufptr, 0, sizeof( Graphic_product ) );
...
/***********************************************************************
   ...
   Description:
      Returns the value of Force_Use_suppl_scans. 

***********************************************************************/
int OB_force_use_supplemental_scans(){

   return( Force_use_suppl_scans );

/* End of OB_force_use_supplemental_scans(). */
}
...
void OB_initialize ( Orpgtat_entry_t *task_table ){
    ...
    for( j = 0; j < N_outs; j++ ){
      ...
      LE_send_msg( GL_INFO, "--->Type: %8d, Timing: %4d, Event_id: %8d\n",
                   Out_list[j].type, Out_list[j].timing, Out_list[j].event_id );
...
int OB_get_elev_cnt( int olind ){
   ...
   if( (Out_list [olind].timing == RADIAL_DATA)  ||
       ...
       (Out_list [olind].timing == REQUEST_DATA) ||
...
static int Output_alert_message_product ( char *buf, int ind, int olind ){
    ...
    /* If the product is to be compressed, compress it here.   Currently
       we do not support compression of Radial Data.  NOTE:  This must be
       done after the UMC call. */
    compress = ORPGPAT_get_compression_type( type );
    if( (compress > 0) && (Out_list[olind].timing != RADIAL_DATA) ){
                                                                                                           
       LE_send_msg( GL_INFO, "Product ID %d To Be Compressed\n", type );
                                                                                                           
       status = RPGC_compress_product( (void *) Buffers[ind].bpt, compress );
       if( status < 0 )
...
static void User_selectable_precip_processing( Prod_header *phd ){
   ...
   if( prod_id == HYUSPACC ){
      ...
      if( last_update_time == HRDB_INIT_VALUE ){
         ...
         current_summary = (Scan_Summary *) RPGC_get_scan_summary( vol_scan_num );
      ...
      if( end_time == last_update_time ){
         ...
         index = RPGC_get_request( phd->g.elev_ind, prod_id, uarray, 10 );
         ...
         for( i = 0; i < index; i++ ){
            ...
            if( (uarray[i].ua_dep_parm_0 == -1) 
                                 &&
                (phd->g.req_params[USP_DURATION] == uarray[i].ua_dep_parm_1) ){
...
int OB_tag_outbuf_wreq( int *bufptr, User_array_t *user_array, int olind ){
   ...
   int ind;
   char *bpt = (char *) bufptr;

   /* Account for the Prod_header .... only on non-radial outputs. */
   if( Out_list[olind].timing != RADIAL_DATA )
      bpt -= sizeof(Prod_header);

   /* Search for the buffer in the list of allocated buffers. */
   for( ind = 0; ind < N_bufs; ind++ ){

      if( Buffers[ind].bpt == bpt )
         break;
...
static int Write_product( char *buf, int ind, int olind, int stream ){
   ...
   /* If product is of timing RADIAL_DATA, product does not include orpg 
      product header. */
   if (Out_list [olind].timing == RADIAL_DATA) {

      len = Out_list [olind].len;
      bpt = buf;

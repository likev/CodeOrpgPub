 ...
 * $Author: steves $
 * $Locker:  $
 * $Date: 2009/03/04 14:54:54 $
 * $Id: rpgc_adpt_data_elems_c.c,v 1.11 2009/03/04 14:54:54 steves Exp $
 * $Revision: 1.11 $
 * $State: Exp $
 */

#define RPGC_ADAPTATION_DATA_ELEMENTS
...
int RPGC_DEAU_callback_fx( int lb_fd, LB_id_t msg_id, int msg_len, char *grp_name );
int RPGC_site_info_callback_fx( void *struct_address );


/********************************************************************

    Description:
       C/C++ interface that adds a new adaptation callback function to the 
    ...
    Return:	
       Returns a identifier for this registration.  This is to be
       used in subsequent calls.  If error occurs, returns -1. 

    Notes:	

********************************************************************/
int RPGC_reg_ade_callback( int (*update) (), void *blk_ptr, char *grp_name,
                           int timing, ...){
   ...
   if( (ret = RPGC_is_ade_callback_reg( update, &status )) >= 0 )
      return (ret);
   ...
   if( timing == ADPT_UPDATE_ON_EVENT ){
      ...
      va_start(ap, timing);            /* Start the variable argument list */
   ...
   /* the group name change, then RPGC_DEAU_callback_fx() is called. */
   Adapt[N_ade].group_name = group_name;

   EN_control( EN_PUSH_ARG, Adapt[N_ade].group_name );
   ret = DEAU_UN_register( Adapt[N_ade].group_name, (void *) RPGC_DEAU_callback_fx );
...
/* End of RPGC_reg_ade_callback() */
}

/*************************************************************************
   Description:
      C/C++ interface that checks if adapt data callback function identified
...
/* End of RPGC_is_ade_callback_reg() */
}

/*******************************************************************

   Description:
      C/C++ interface that calls the adaptation data callback function 
      based on ID of registered callback.
...
int RPGC_read_ade( int *callback_id, int *status ){
   ...
   *status = 0;
   ...
   if( (ind = RPGC_is_ade_callback_reg( Adapt[id].f_ptr, &success )) < 0 ){
   ...
   if( Adapt[id].dea_modified == 0 )
     return -1;

   /* Unset dea flag. */
   Adapt[id].dea_modified = 0;

   /* Call the callback function. */
   if( Adapt[id].f_ptr( Adapt[id].block_address ) >= 0 ){
...
/* End of RPGC_read_ade() */
}

/********************************************************************

    Description: 
       C/C++ interface that calls all the registered callback functions.
...
/* End of RPGC_update_all_ade() */
}

/********************************************************************

   Description:
      C/C++ interface that retrieves value(s) from the database given
      "alg_name", "valid_id", and a receiving buffer.
   ...
   Returns:
      Returns -1 on error, or number of values on success.
...
/* End of RPGC_ade_get_values() */
}

/********************************************************************
   Description:
      C/C++ interface that retrieves string(s) from the database given
      "alg_name", "valid_id", and a pointer to string to receive
      the string(s).
...
/* End of RPGC_ade_get_string_values() */
}

/********************************************************************

   Description:
      C/C++ interface that retrieves number of value(s) for the "alg_name"
      and "valid_id" variables.
...
/* End of RPGC_ade_get_number_of_values() */
}

/********************************************************************

   Description:
      C/C++ interface that registers a struct to get the latest site
      information adaptation data.

   Inputs:
      struct_address - pointer to a site_adapt_t struct
...
********************************************************************/
int RPGC_reg_site_info( void *blk_ptr ){
   ...
   if( ret >= 0 )
      ret = RPGC_reg_ade_callback( RPGC_site_info_callback_fx, blk_ptr,
                                   Site_info_name, temp );
...
/* End of RPGC_reg_site_info() */
}

/********************************************************************

   Description:
      C/C++ interface that registers a struct to get the latest color
      table adaptation data.

   Inputs:
      buf - pointer to a Coldat_t struct
      timing - update timing (ADPT_UPDATE_BOE, ADPT_UPDATE_BOV,
               ADPT_UPDATE_ON_CHANGE, ADPT_UPDT_WITH_CALL, or
...
int RPGC_reg_color_table( void *buf, int timing, ... ){
   ...
   if( timing == ADPT_UPDATE_ON_EVENT ){
       ...
       return( RPGC_reg_adpt( id, buf, timing, event_id ) );
   }

   return( RPGC_reg_adpt( id, buf, timing ) ); 

/* End of RPGC_reg_color_table() */
}

/********************************************************************

   Description:
      C/C++ interface that registers a struct to get the RDACNT
      adaptation data.

   Inputs:
      buf - pointer to a Rdacnt struct
      timing - update timing (ADPT_UPDATE_BOE, ADPT_UPDATE_BOV,
               ADPT_UPDATE_ON_CHANGE, ADPT_UPDT_WITH_CALL, or
               ADPT_UPODATE_ON_EVENT).
      event_id - event id if timing is ADPT_UPDTAE_ON_EVENT (optional)
...
/*************************************************************************
   ...
   Inputs:
      grp_name - name of adaptation data group
      Other inputs are required for a callback function and aren't used. 
      Check man pages of deau, lb, and en for details.

/********************************************************************
 ...
 Description:
    This module contains the RPG Library routines RPG_wait_act 
    (Wait for Activation) RPG_wait_for_any_data and their supporting 
    functions for porting old RPG product/algorithm tasks.

 Notes:
    Modules beginning with RPG_ are public functions.  Modules beginning
    with WA_ are library public functions.  All other functions
    are private.
 ...
 * $Id: rpg_wait_act.c,v 1.76 2014/04/25 17:22:32 steves Exp $
 * $Revision: 1.76 $
 * $State: Exp $
 */
...
static int Message_mode = 0;		/* Flag, if set, indicates task is in additional
					   message mode. */

static int Use_suppl_scans = 0;		/* Use of supplemental scans flag. */
...
static void Abort_replay_request( unsigned int vol_seq_num );
...
int RPG_wait_for_task( char *task_name ){
   ...
   char *loc, name[256]; 
   int length, task_state;

   int dollar_sign = (int) '$';


   /* Terminate the "task_name" string with NULL character.  The
      input task name string is assumed terminated with '$'.  If
      termination string not found, simply return.  */
   loc = strchr( task_name, dollar_sign ); 
   if( loc == NULL )
      return 0;

   length = loc - task_name;
   memcpy( name, task_name, length );
   *(name + length) = '\0'; 
   ...
   while(1){
      ...
      if( ORPGMGR_get_task_status( (char *) &name[0], 0, &task_state ) < 0 )
         return 0;
...
/********************************************************************
    ...
    Inputs:
	 what_for - either WAIT_DRIVING_INPUT or WAIT_ANY_INPUT;
...
********************************************************************/
int RPG_wait_act( fint *wait_for ){
    ...
    /* This module only proceeds with argument WAIT_DRIVING_INPUT. */
	...
	output_requested = Output_requested();
        ...
        /* For BASEDATA and BASEDATA_ELEV inputs, check if data is needed.  
           For all other types of inputs, always returns DATA_NEEDED. */
        ...
        if( (AP_abort_flag( UNKNOWN ) == TRUE)
           ...
           PS_message( "Wait For Activation Aborting Outputs\n" );
...
/*******************************************************************
   ...
   Outputs:
      Set the "allow_suppl_scans" flag.

*******************************************************************/
fint RPG_allow_suppl_scans( fint *allow_suppl_scans ){

   *allow_suppl_scans = Use_suppl_scans;
   return 0;

/* End of RPG_allow_suppl_scans() */
}
...
********************************************************************/
int RPG_wait_for_any_data( fint *wait_for, fint *input_avail ){

   /* Wait for any of the registered inputs to arrive. */
   return( WA_wait_for_any_data( wait_for, input_avail, 
                                 EVT_WAIT_FOR_DATA ) );

/* End of RPG_wait_for_any_data() */
}
...
void WA_initialize( Orpgtat_entry_t *task_entry ){
   ...
   output_radial_data = OB_use_supplemental_scans();
...
/********************************************************************
   ...
   Returns:	
      Status field of the current radial input.
...
int WA_check_data ( int buffer_ind ){
    ...
    /* Do first time initialization for replay tasks consuming radial inputs
       or after if the replay task was previously aborted. */
    ...
    /* Set the volume sequence number in VI modulei and PRQ modules. */
    ...
    if( (Bd_status == GOODBEL) || (Bd_status == GOODBVOL) ){
	   ...
	   Test.new_elev = TRUE;
        ...
        if( Test.new_vol == TRUE ){
           ...
           Test.n_exp_cuts = VI_get_num_elevations( Test.vcp );
	...
	if( clear_abort_flag ){
            ...
            /* Print out additional info if in message mode. */
            if( Message_mode ){

               if( Resume_time == NEW_DATA )
                  PS_message( "Processing Resumed For NEW_DATA\n" );
	       else if( Test.new_elev && Resume_time == NEW_ELEVATION) 
                  PS_message( "Processing Resumed For NEW_ELEVATION (new_vol = %d)\n",
                              Test.new_vol );
	       else if( Test.new_vol && Resume_time == NEW_VOLUME )
                  PS_message( "Processing Resumed For NEW_VOLUME\n" );
    ...
    if( (!Waiting_for_activation) && (ret != NORMAL) )
       ...
       LE_send_msg( GL_INFO,
                    "WA_check_data Ret Bad Status To WA_wait_act (%d)\n", ret );
...
int WA_data_filtering (In_data_type *idata, int buffer_ind){
   ...
   /* If input data class ID is BASEDATA, compare the msg_type field 
      of the radial with the type mask field provided by the PAT. */
  if( (idata->data_id == BASEDATA)
                      ||
       (idata->data_id == RAWDATA) 
                       ||
       (ORPGPAT_get_class_id( alias_id ) == BASEDATA) 
                         ||
       (ORPGPAT_get_class_id( alias_id ) == RAWDATA) ){
      ...
      /* Check if this is a supplemental cut. If so, do not allow
         processing.  This is a conscious decision. */
      if( (has_suppl_scan) && (is_suppl_scan) )
         return (DATA_NOT_NEEDED);

      if ( bhd->msg_type & idata->type_mask )
         return (DATA_NEEDED);
   ...
   /* If input data class ID is BASEDATA_ELEV, compare the type field of the 
      ...
      elev_prod = (Compact_basedata_elev *) (idata->buf[buffer_ind] + sizeof(Prod_header));
      radial = (Compact_radial *) &elev_prod->radial[0];

      /* Check is this is supplemental cut.  If so, this task is not allowed
         to process it. */
      ...
      if( elev_prod->type & idata->type_mask )
         return(DATA_NEEDED);
         ...
         if( Status.updated == LB_TRUE ){
            ...
            if( ret > 0 || ret == LB_BUF_TOO_SMALL ){
               ...
               if( (int) prod_header.g.vol_num >= Vol_seq_num_wait_any ){
                  ...
                  /* Set the volume sequence number in VI module. */
	          ...
	          if ( (new_vol && (Resume_time == NEW_VOLUME))
                     ...
                     /* Print out additional information if in message mode. */
                     if( Message_mode ){

	                if( new_vol && (Resume_time == NEW_VOLUME) )
                           PS_message( "Clearing Abort flag for NEW_VOLUME\n" );
	                else if( Resume_time == NEW_DATA )
                           PS_message( "Clearing Abort flag for NEW_DATA\n" );
                        else if( new_elev && (Resume_time == NEW_ELEVATION) )
                           PS_message( "Clearing Abort flag for NEW_ELEVATION\n" );
...
int WA_set_output_info( char *outbuf, int olind ){
   ...
   if( (Vtest.vcp == UNDEFINED)
      ...
      num_rda_elevs = VI_get_num_elevations( Vtest.vcp );

      /* We need to subtract 1 in function call since argument assumes 0 indexed. */
      Vtest.n_exp_cuts = VI_get_rpg_elevation_num( Vtest.vcp, num_rda_elevs-1 );
...
static int Check_data_prod ( int buffer_ind ){
    ...
    Prod_header *phd;
    int escape, ret, bad_input, data_needed;
    time_t tm;
    ...
    /* Set the volume sequence number in VI and PRQ modules. */
    VI_set_vol_seq_num( Vol_seq_num_wait_all );
    PRQ_set_vol_seq_num( Vol_seq_num_wait_all );
    ...
    /* Input data indicate new volume scan if any of the following 
       ...
       (1) the driving input for this task is elevation data
           ...
           AND this is the first product generated in the current 
           volume scan (this last condition supports consumers of 
           elevation data for which the first product may not 
           correspond to the lowest elevation scan).  For driving
           input of REFLDATA_ELEV or COMBBASE_ELEV, check if the 
           data is also needed. 
    ...
    if( (Driving_input_type == ELEVATION_DATA)
                            &&
        (((phd->g.elev_ind == 1) && (phd->elev_cnt == 1))
                            ||
	...
	if ((Resume_time != NEW_VOLUME) ||
	   ...
	   if( Resume_time != NEW_VOLUME){
              ...
              /* Print out additional information if in message mode. */
              if( Message_mode )
                 PS_message( "Clearing Abort Flag For Resume_time != NEW_VOLUME\n" );
...
/********************************************************************
   ...
   Description: 
      This function initializes variables in this module. It also 
      verifies whether the input types satisfy the 
      assumptions described in support.doc.
...
static void Initialize (){
    ...
    /* Set Driving_input_type and and verify the input data timing. */
...
static void Wait_for_any_init( int *wait_for ){
   ...
   if( *wait_for == WAIT_ANY_INPUT ){ 
      ...
      /* Call RPG_wait_act with input wait type of WAIT_ANY_INPUT. An 
         argument of WAIT_ANY_INPUT causes initialization of input data 
         type list, then an immediate return. */
      RPG_wait_act( wait_for );
         ...
         /* The driving input is in the product database as warehoused
            ...
            if( status == NORMAL ){
               ...
               if( msg_id == 0 ){
                  ...
                  Abort_replay_request( (unsigned int) requests->vol_seq_num );
...
static int Check_radial_replay_availability( Prod_request *requests ){
   ...
   while( abs( (int) (vol_seq_num - requests->vol_seq_num)) <= 1 ){
...
static void Abort_replay_request( unsigned int vol_seq_num ){
   ...
   LE_send_msg( GL_ERROR, "Unable To Satisfy Request\n");
   LE_send_msg( GL_ERROR, "Input Data Not Available For Volume Seq. # %d.\n",
                vol_seq_num );

   /* Set the volume sequence number of the aborted volume scan. */
   AP_set_aborted_volume( (int) vol_seq_num );

   /* Abort the product request. */
   AP_abort_outputs( PGM_REPLAY_DATA_UNAVAILABLE );
   ...
   if( (Alg_control_flg = 
	 ...
	 /* Abort and wait until next volume to resume processing. */
	    ...
	    ret = TERMINATE;
         ...
         /* For radial timing, abort when elevation and volume based tasks
            are told to abort for new new volume. */
   ...
   if( test->new_vol ){
	 ...
	 test->azi_num = bhd->azi_num;
      ...
      if( test->end_elev ){
         ...
         if( test->rpg_cut_num != UNDEFINED ){
            ...
            if( test->rpg_cut_num != bhd->rpg_elev_ind ){
               ...
               if( (test->rpg_cut_num + 1) != bhd->rpg_elev_ind ){
                  ...
                  LE_send_msg (GL_INFO,
	               "New Elev # Out Of Seq Error (%d+1 != %d) @ EL/AZM %4.1f/%5.1f\n",
         ...
         /* This handles the case of new elevation without end of previous elevation
            and new elevation RPG elevation index is different. */
         LE_send_msg (GL_INFO,
	      "New Elev # Out Of Seq Error (%d+1 != %d) @ EL/AZM %4.1f/%5.1f\n",
	      test->rpg_cut_num, bhd->rpg_elev_ind, bhd->elevation, bhd->azimuth);
...
static int Radial_seq_check_pass_2( Rad_test_t *test, Base_data_header *bhd ){
   ...
   if( test->azi_num + 1 != bhd->azi_num ){
      ...
      if ( test->azi_num != UNDEFINED && !test->new_scan ){
         ...
         LE_send_msg (GL_INFO,
	     "Azimuth # Seq Error (%d+1 != %d) @ EL/AZM %4.1f/%5.1f\n", 
	      test->azi_num, bhd->azi_num, bhd->elevation, bhd->azimuth);
...
static int Check_avset(){
    ...
    int reason = PGM_PROD_NOT_GENERATED;
    int olind;
    ...
    if( (Inp_list[DRIVING].timing == RADIAL_DATA)
        ...
        if( (Test.vcp != UNDEFINED) && (Test.end_vol == TRUE) ){
            ...
            if( Test.n_cuts < Test.n_exp_cuts ){
                ...
                for( olind = 0; olind < N_outs; olind++ ){
                    ...
                    if( Out_list[olind].timing == ELEVATION_DATA ){
                        ...
                        LE_send_msg( GL_INFO, "--->Aborting Product Type: %d\n",
                                     Out_list[olind].type );
                        RPG_abort_datatype_processing( &Out_list[olind].type, &reason );
            ...
            /* Reset to UNDEFINED .... it will be initialized when it starts processing
               input data. */
            Test.vcp = UNDEFINED;
    ...
    /* For Radial or Elevation driving input and Elevation-based outputs. */
    else if( ((Task_type == TASK_ELEVATION_BASED)
                          ||

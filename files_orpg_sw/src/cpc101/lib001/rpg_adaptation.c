 ...
 * $Author: steves $
 * $Locker:  $
 * $Date: 2006/04/24 19:17:32 $
 * $Id: rpg_adaptation.c,v 1.21 2006/04/24 19:17:32 steves Exp $
 * $Revision: 1.21 $
 * $State: Exp $
 */
...
/* Only include in the RPG library. */
#ifdef RPG_LIBRARY

#define PRODSEL_BLOCKS        3
static Legacy_adapt_t prodsel_block[ PRODSEL_BLOCKS ] = {

                      { ORPGDAT_ADAPT 			/* data store id */,
                        "Layer Product Parameters"      /* block name */,
                        sizeof( int ) 			/* offset */,
			sizeof( layer_prod_params_t)	/* length */
		      },

                      { ORPGDAT_ADAPT 	/* data store id */,
                        "RCM Product Parameters"      /* block name */,
                        sizeof( int ) + 
                        sizeof( layer_prod_params_t ) /*  offset */,
			sizeof( rcm_prod_params_t )   /*  length */
		      },

                      { ORPGDAT_ADAPT 			/* data store id */,
                        "VAD/RCM Heights"      		/* block name */,
                        sizeof( int ) + 
                        sizeof( layer_prod_params_t ) +
                        sizeof( rcm_prod_params_t ) 	/*  offset */,
			sizeof( vad_rcm_heights_t )	/*  length */
		      }
		      };
...
#define ENVIRON_BLOCKS        1
static Legacy_adapt_t environ_block[ENVIRON_BLOCKS] = {
...
void Adapt_lb_notify_callback( int fd, LB_id_t msgid, int msg_info,
                               void *arg );
void Adapt_on_change_callback( orpgadpt_event_t *event );
...
********************************************************************/
int RPG_reg_adpt( fint *id, char *buf, fint *timing, ...){

    int status;
    va_list ap;

    /* Check if adaptation block alread registered. */
    if( RPG_is_adapt_block_registered( id, &status ) >= 0 )
       return (0);
    ...
    if( *timing != ADPT_UPDATE_BOE && *timing != ADPT_UPDATE_BOV &&
        *timing != ADPT_UPDATE_ON_CHANGE && *timing != ADPT_UPDATE_WITH_CALL && 
...
*************************************************************************/
int RPG_is_adapt_block_registered( fint *block_id, fint *status ){
...
/* End of RPG_is_adapt_block_registered() */
}
...
********************************************************************/
int RPG_read_adapt_block( fint *block_id, fint *status ){
   ...
   *status = 0;
   ...
   if( (ind = RPG_is_adapt_block_registered( block_id, &success )) < 0 )
      return 0;
...
/********************************************************************
   ...
   Description: 
      This function adds a new adaptation block to the registered 
      adaptation block list. Old version.
   ...
   Returns:	
      The return value is never used. On failure this 
...
int register_adapt(int *id, char *buf){
    ...
    tmp = ADPT_UPDATE_BOE;
    return (RPG_reg_adpt (id, buf, &tmp));

/* End of register_adapt() */
}
...
/*******************************************************************
   ...
   Description:
      This is used to read adaptation data using the ORPGADPT 
      suite of functions.
...
static int Read_subblock( Legacy_adapt_t *block, char *start_addr,
   ...
   if( block->data_store_id == ORPGDAT_ADAPTATION ){
      ...
      /* Get length of adaptation data block. */
      data_length = ORPGADPT_get_data_length( block->block_name );
      if( data_length > 0 ){

         /* Read adaptation data block. */
         ORPGADPT_get_data( block->block_name,
                            start_addr + block->block_offset, block->length);

         /* Check for read errors. */
         if( ORPGADPT_error_occurred() ){

            ORPGADPT_log_last_error( GL_INFO, ORPGADPT_CLEAR_ERROR );
            return (-1);
         ...
         LE_send_msg( GL_INFO, "Adaptation Object %s Does Not Exist\n",
                      block->block_name );
         if( ORPGADPT_error_occurred() )
            ORPGADPT_log_last_error( GL_INFO, ORPGADPT_CLEAR_ERROR );
         return (-1);
...
#ifdef RPG_LIBRARY
       case PRODSEL:
          LE_send_msg( GL_INFO, "Registered For PRODSEL Adaptation Data\n" );
          Adapt[ind].block = prodsel_block;
          Adapt[ind].num_blocks = PRODSEL_BLOCKS;
          break;
#endif
...
#ifdef RPG_LIBRARY
       case ENVIRON:
          LE_send_msg( GL_INFO, "Registered For ENVIRON Adaptation Data\n" );
          Adapt[ind].block = environ_block;
          Adapt[ind].num_blocks = ENVIRON_BLOCKS;
          break;
#endif
...
/**********************************************************************
   ...
   Description:
      Callback function to register changes in adaptation data.  Used
      in conjuction with ORPGADPT library.

   Inputs:
      event - pointer to change event data.
...
void Adapt_on_change_callback( orpgadpt_event_t *event ){
   ...
   /* Set the changed flag for this adaptation data block. */
   Adapt_block *ptr = (Adapt_block *) event->user_data;
   ptr->changed = TRUE;

/* End of Adapt_on_change_callback() */
}
...
void Register_for_updates( int ind, int subblock_ind ){
   ...
   if( Adapt[ind].block[subblock_ind].data_store_id == ORPGDAT_ADAPTATION ){
      ...
      /* If data store is not ORPGDAT_ADAPTATION, use ORPGADPT_on_change. */
      if( !ORPGADPT_on_change( Adapt[ind].block[subblock_ind].block_name,
                   Adapt_on_change_callback, &Adapt[ind] ) )
         PS_task_abort( 
               "ORPGADPT_on_change Failed For Adapt File %d",

 ...
 * $Id: cn_read_messages.c,v 1.1 2014/05/13 15:01:27 steves Exp $
 * $Revision: 1.1 $
 * $State: Exp $
 *
 */
...
extern float NYQUIST_VEL;

static int   Rda_status_updated = 0; /* RDA status updated flag */
...
void RM_determine_msg_hdr_offset (char *buf, int *offset, int *cm_type, 
   ...
   if( input_data_id == CLDM_SR_BASEDATA ){
...
/***************************************************************************
   ...
   Input: Rda_status_updated        - RDA status updated flag
...
int RM_read_next_msg (char **buf, int input_data_id) {
   ...
   char *tmp_buf;
      ...
      /* If RDA Status Data Updated Event Received, process it now. */

   if (Rda_status_updated) {

      Rda_status_updated = 0;
      ...
      *buf = tmp_buf;
      ...
      if ((ret > 0) && (input_data_id == CLDM_SR_BASEDATA))
         ret = Convert_to_msg31 (buf, 0);
...
/***************************************************************************
   ...
   Description: Set the recombined RDA Status msg updated flag
        ...
        Output: Rda_status_updated - recombined RDA status updated flag
...
static int Convert_to_msg31 (char **buf_ptr, int rda_channel) {
  ...
  if( NYQUIST_VEL > 0.0 )
     rad_data->nyquist_vel = NYQUIST_VEL*100.0;
  else 
  ...
  if ((m->n_dop_bins > 0) && (m->vel_offset != 0)) {
    ...
    /* Convert the Nyquist velocity to scaled/biased units. */
    nyquist_p = (int) (vel_data->offset + (NYQUIST_VEL*vel_data->scale));
    nyquist_n = (int) (vel_data->offset - (NYQUIST_VEL*vel_data->scale));
    nyquist_2p = (int) (2.0*NYQUIST_VEL)*vel_data->scale;

    data = (Moment_t *) (buf+m->vel_offset);

    /* Check to make sure the Nyquist Velocity is set. */
    if( NYQUIST_VEL > 0.0 ){
       ...
       for (i=0; i < m->n_dop_bins; i++){
          ...
          if( data[i] > nyquist_p ){
             ...
             while( data[i] > nyquist_p )
                data[i] -= nyquist_2p;

          }

          if( (data[i] > BASEDATA_RDRNGF) && (data[i] < nyquist_n) ){

             while( data[i] < nyquist_n )
                data[i] += nyquist_2p;

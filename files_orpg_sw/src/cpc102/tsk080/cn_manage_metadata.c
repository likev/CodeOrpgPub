 ...
 * $Id: cn_manage_metadata.c,v 1.1 2014/05/13 15:01:27 steves Exp $
 * $Revision: 1.1 $
 * $State: Exp $
 *
 *
...
void MM_clear_metadata_buf (void) {
   ...
   memset (Metadata_buffer, 0, LDM_META_DATA_SIZE);
      ...
      /* reset the msg segment counters array */
...
void MM_process_metadata_msg (char *msg_buf, msg_data_t msg_data) {
   ...
   int i;
      ...
      /* when the first msg is received for each metadata msg type,
   ...
   if (msg_data.msg_hdr.seg_num == 1) {
      ...
      LE_send_msg (GL_INFO, "Writing Metadata msg %d to the Metadata buffer",
   ...
   switch (msg_data.msg_hdr.type) {
      ...
      /* if more msg segments for a msg are recieved than are reserved in the 
         Metadata buffer, reject the whole msg */
...
void MM_write_metadata (msg_data_t *msg_data) {
   ...
   WR_process_record (Metadata_buffer, LDM_META_DATA_SIZE, msg_data);
...
static void Add_msg_to_metadata_buf (int msg_idx, int rda_conf) {
   ...
   memcpy ((char *) &msg_hdr, tmp_buf + msg_offset, msg_hdr_size);
      ...
      /* validate the metadata msg read from the LB */
   ...
   Copy_data_to_buffer (tmp_buf, msg_hdr, actual_num_msg_segs, msg_data_len,
      ...
      /* update the seg counter for this msg type */

   Metadata_msgs_seg_cntrs [msg_idx] = actual_num_msg_segs;

   LE_send_msg (GL_INFO, 
      "Msg type %d (byte size: %d;  # segments: %d) read from LB %d written to Metadata buffer",
...
/*   LE_send_msg (GL_INFO, "Clearing Metadata msg block for msg type %d", msg_type); */
   ...
   switch (msg_type) {
      ...
      /* reset the counter int msg segment counter array */
...
/********************************************************************************
    ...
    Description: Validates the Metadata buffer by ensuring that all required
                 ...
                 to the Metadata buffer. If the segment counter is 0, then the 
...
static void Validate_metadata (int rda_config) {
   int idx;
      ...
      /* check the msg segment counter array to ensure every msg type

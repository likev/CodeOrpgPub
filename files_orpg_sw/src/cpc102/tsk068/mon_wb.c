 ...
 * $Author: steves $
 * $Locker:  $
 * $Date: 2013/07/22 18:29:48 $
 * $Id: mon_wb.c,v 1.72 2013/07/22 18:29:48 steves Exp $
 * $Revision: 1.72 $
 * $State: Exp $
 */
...
static int ProcessLgcyRdaStatusMsg(short *msgPtr)
{
   ...
   if ( VerboseLevel >= MONWB_MODERATE )
   {
      ...
      {
         fprintf(stderr, MONWB_FLOAT_FMT_STRING, BlankString, "Refl Calib Corr. (scaled)",
      ...
      if( map != NULL )
      {
         ...
         for( i = 1; i <= 360; i++ )
         {
            ...
            if( (num_zones <= 0) || (num_zones > MAX_RANGE_ZONES_ORDA) ){
               ...
               fprintf( stderr, "El/Az %3d/%3d # Zones Invalid: %6d\n",
...
static int PrintGenRadMoment( char* mom_id, char* data_ptr )
{
   ...
   { 
          ...
          (PrintMomentDataFlag == MONWB_TRUE))
      {
         ...
         {
            fprintf(stderr, MONWB_FLOAT_FMT_STRING_ARRAY, BlankString, "Value",
      ...
      switch ( mom_data_ptr->data_word_size )
      {
         case 8:
                ...
                (PrintMomentDataFlag == MONWB_TRUE))
            {
               for ( index = 0; index < mom_data_ptr->no_of_gates; index++ )
               {
                  fprintf(stderr, MONWB_UINT_FMT_STRING_ARRAY, BlankString, "Value",

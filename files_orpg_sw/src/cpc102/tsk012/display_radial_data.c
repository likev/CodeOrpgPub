/************************************************************************
 *	Module: display_radial_data.c					*
 *	Description: This module is used to display a polar product	*
 *		     in the display window used by the NEXRAD Product	*
 *		     Display Tool (XPDT).				*
 ************************************************************************/
 ...
 * $Id: display_radial_data.c,v 1.5 2014/03/18 19:00:42 jeffs Exp $
 * $Revision: 1.5 $
 * $State: Exp $
 */
...
#define	DEGRAD	(3.14159265/180.0) /* degrees to radians cnversion */
...
extern	float	value_min; /* minimum data value */
extern	float	value_max; /* maximum data value */
...
/************************************************************************
 *	Description: This function dispays a radial product in the XPDT	*
 *		     display window.					*
 *									*
 *	Input:  NONE							*
 *	Output: NONE							*
...
/*	Set the clip window.					*/
	...
	if (color_step <=0) {
	    ...
	    printf ("WARNING: check number of data levels -> assume 16\n");
	    color_step = 1;

	}

	for (i=0;i<radial_image->data_elements;i++) {

	    Range [i] = i*xy_azran_reso [attribute->product_code][0];
 ...
 *	value as a unique color in the display.				*/
	...
	for (beam=0;beam<radial_image->number_of_radials;beam++) {

	    azimuth1 = radial_image->azimuth_angle [beam] - 0.5;
	    azimuth2 = azimuth1 + radial_image->azimuth_width [beam] + 0.5;
 ...
 *	occurs.								*/
		...
		Color = radial_image->radial_data[beam][i] * color_step;

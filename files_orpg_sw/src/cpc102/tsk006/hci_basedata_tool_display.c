/************************************************************************
 *									*
 *	Module:  hci_basedata_tool_display.c				*
 *									*
 ...
 * $Author: ccalvert $
 * $Locker:  $
 * $Date: 2009/04/07 15:19:14 $
 * $Id: hci_basedata_tool_display.c,v 1.4 2009/04/07 15:19:14 ccalvert Exp $
 * $Revision: 1.4 $
 * $State: Exp $
 */
...
void hci_basedata_tool_draw_bin( float range, float value )
{
  ...
  {
    if( !Doppler_data_flag )
    {
      color = 0;
    }
    else
    {
      if( value > 999 ){ color = PRODUCT_COLORS - Max_color_adjust+1; }
      else{ color = 0; }
    }
  }
  else

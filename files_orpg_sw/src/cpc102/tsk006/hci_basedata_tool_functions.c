/************************************************************************
 *									*
 *	Module:  hci_basedata_tool_functions.c				*
 *									*
 ...
 * $Author: steves $
 * $Locker:  $
 * $Date: 2011/03/03 23:32:33 $
 * $Id: hci_basedata_tool_functions.c,v 1.6 2011/03/03 23:32:33 steves Exp $
 * $Revision: 1.6 $
 * $State: Exp $
 */
...
static float Reflectivity_16bit_LUT[NUM_16BIT_ELEMENTS];
static float Prev_reflectivity_16bit_scale = 2.0;
static float Prev_reflectivity_16bit_offset = 66.0;
...
static float Velocity_low_16bit_LUT[NUM_16BIT_ELEMENTS];
static float Prev_velocity_low_16bit_scale = 1.0;
static float Prev_velocity_low_16bit_offset = 129.0;
...
static float Velocity_high_16bit_LUT[NUM_16BIT_ELEMENTS];
static float Prev_velocity_high_16bit_scale = 2.0;
static float Prev_velocity_high_16bit_offset = 129.0;
...
static float Spectrum_width_16bit_LUT[NUM_16BIT_ELEMENTS];
static float Prev_spectrum_width_16bit_scale = 2.0;
static float Prev_spectrum_width_16bit_offset = 129.0;
...
static float Diff_reflectivity_16bit_LUT[NUM_16BIT_ELEMENTS];
static float Prev_diff_reflectivity_16bit_scale = 16.0;
static float Prev_diff_reflectivity_16bit_offset = 128.0;
static float Diff_phase_8bit_LUT[NUM_8BIT_ELEMENTS];
static float Prev_diff_phase_8bit_scale = 2.0;
static float Prev_diff_phase_8bit_offset = 2.8361;
...
static float Diff_correlation_16bit_LUT[NUM_16BIT_ELEMENTS];
static float Prev_diff_correlation_16bit_scale = 300.0;
static float Prev_diff_correlation_16bit_offset = -60.5;
...
void build_diff_reflectivity_16bit_LUT( float n_offset, float n_range );
...
void build_diff_correlation_16bit_LUT( float n_offset, float n_range );
...
void hci_basedata_tool_init()
{
  ...
  build_reflectivity_16bit_LUT( Prev_reflectivity_16bit_offset,
                               Prev_reflectivity_16bit_scale );
  build_velocity_high_8bit_LUT( Prev_velocity_high_8bit_offset,
                               Prev_velocity_high_8bit_scale );
  build_velocity_high_16bit_LUT( Prev_velocity_high_16bit_offset,
                               Prev_velocity_high_16bit_scale );
  build_velocity_low_8bit_LUT( Prev_velocity_low_8bit_offset,
                               Prev_velocity_low_8bit_scale );
  build_velocity_low_16bit_LUT( Prev_velocity_low_16bit_offset,
                               Prev_velocity_low_16bit_scale );
  build_spectrum_width_8bit_LUT( Prev_spectrum_width_8bit_offset,
                               Prev_spectrum_width_8bit_scale );
  build_spectrum_width_16bit_LUT( Prev_spectrum_width_16bit_offset,
                               Prev_spectrum_width_16bit_scale );
  build_diff_reflectivity_8bit_LUT( Prev_diff_reflectivity_8bit_offset,
                               Prev_diff_reflectivity_8bit_scale );
  build_diff_reflectivity_16bit_LUT( Prev_diff_reflectivity_16bit_offset,
                               Prev_diff_reflectivity_16bit_scale );
  build_diff_phase_8bit_LUT( Prev_diff_phase_8bit_offset,
                               Prev_diff_phase_8bit_scale );
  ...
  build_diff_correlation_16bit_LUT( Prev_diff_correlation_16bit_offset,
                               Prev_diff_correlation_16bit_scale );
    ...
    for( i = 0; i < Hdr->no_moments; i++ )
    {
      ...
      if( strcmp( moment_name, "DZDR" ) == 0 )
      {
        ...
        else
        {
          ...
          if( ghdr->offset != Prev_diff_reflectivity_16bit_offset ||
              ghdr->scale != Prev_diff_reflectivity_16bit_scale )
          {
            build_diff_reflectivity_16bit_LUT( ghdr->offset, ghdr->scale );
          }
        }
      ...
      else if( strcmp( moment_name, "DPHI" ) == 0 )
      {
        ...
        if( ghdr->data_word_size == IS_8BIT_DATA )
        {
          ...
          if( ghdr->offset != Prev_diff_phase_8bit_offset ||
              ghdr->scale != Prev_diff_phase_8bit_scale )
          {
            build_diff_phase_8bit_LUT( ghdr->offset, ghdr->scale );
          }
        }
        else
        {
      ...
      else if( strcmp( moment_name, "DRHO" ) == 0 )
      {
        ...
        {
          Diff_correlation_ptr = &ghdr->gate;
          if( ghdr->offset != Prev_diff_correlation_16bit_offset ||
              ghdr->scale != Prev_diff_correlation_16bit_scale )
          {
            build_diff_correlation_16bit_LUT( ghdr->offset, ghdr->scale );
          }
        }
...
float hci_basedata_tool_value( int u_value, int moment )
{
  ...
  switch( moment )
  {
    case SPECTRUM_WIDTH :
      ...
      {
        value = Spectrum_width_16bit_LUT[ u_value ];
      }
      else
      {
    ...
    case VELOCITY :
      switch( hci_basedata_tool_velocity_resolution() )
      {
        case DOPPLER_RESOLUTION_LOW :
          if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
          {
            value = Velocity_low_16bit_LUT[ u_value ];
          }
          else
          {
        ...
        default :
          if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
          {
            value = Velocity_high_16bit_LUT[ u_value ];
          }
          else
          {
    ...
    case DIFF_REFLECTIVITY :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        value = Diff_reflectivity_16bit_LUT[ u_value ];
      }
      else
      {
    ...
    case DIFF_PHASE :
      ...
      {
        value = Diff_phase_8bit_LUT[ u_value ];
      }
      break;
    ...
    case DIFF_CORRELATION :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        value = Diff_correlation_16bit_LUT[ u_value ];
      }
      else
      {
    ...
    default :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        value = Reflectivity_16bit_LUT[ u_value ];
      }
      else
      {
...
float hci_basedata_tool_value_min( int moment )
{
  float value = 0.0;
  ...
  switch( moment )
  {
    case VELOCITY :
      switch( hci_basedata_tool_velocity_resolution() )
      {
        case DOPPLER_RESOLUTION_LOW :
          if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
          {
            value = Velocity_low_16bit_LUT[ INDEX_OF_FIRST_DATA ];
          }
          else
          {
            value = Velocity_low_8bit_LUT[ INDEX_OF_FIRST_DATA ];
          }
          break;
        ...
        default:
          if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
          {
            value = Velocity_high_16bit_LUT[ INDEX_OF_FIRST_DATA ];
          }
          else
          {
            value = Velocity_high_8bit_LUT[ INDEX_OF_FIRST_DATA ];
          }
          break;
      }
      break;
    ...
    case SPECTRUM_WIDTH :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        value = Spectrum_width_16bit_LUT[ INDEX_OF_FIRST_DATA ];
      }
      else
      {
        value = Spectrum_width_8bit_LUT[ INDEX_OF_FIRST_DATA ];
      }
      break;
    ...
    case DIFF_REFLECTIVITY :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        value = Diff_reflectivity_16bit_LUT[ INDEX_OF_FIRST_DATA ];
      }
      else
      {
        value = Diff_reflectivity_8bit_LUT[ INDEX_OF_FIRST_DATA ];
      }
      break;
    ...
    case DIFF_PHASE :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        value = Diff_phase_16bit_LUT[ INDEX_OF_FIRST_DATA ];
      }
      else
      {
        value = Diff_phase_8bit_LUT[ INDEX_OF_FIRST_DATA ];
      }
      break;
    ...
    case DIFF_CORRELATION :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        value = Diff_correlation_16bit_LUT[ INDEX_OF_FIRST_DATA ];
      }
      else
      {
        value = Diff_correlation_8bit_LUT[ INDEX_OF_FIRST_DATA ];
      }
      break;
    ...
    default :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        value = Reflectivity_16bit_LUT[ INDEX_OF_FIRST_DATA ];
      }
      else
      {
        value = Reflectivity_8bit_LUT[ INDEX_OF_FIRST_DATA ];
      }
      break;
  }
...
float hci_basedata_tool_value_max( int moment )
{
  float value = 0.0;
  ...
  switch( moment )
  {
    case VELOCITY :
      switch( hci_basedata_tool_velocity_resolution() )
      {
        case DOPPLER_RESOLUTION_LOW :
          if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
          {
            value = Velocity_low_16bit_LUT[ INDEX_OF_LAST_16BIT_DATA ];
          }
          else
          {
            value = Velocity_low_8bit_LUT[ INDEX_OF_LAST_8BIT_DATA ];
          }
          break;
        ...
        default:
          if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
          {
            value = Velocity_high_16bit_LUT[ INDEX_OF_LAST_16BIT_DATA ];
          }
          else
          {
            value = Velocity_high_8bit_LUT[ INDEX_OF_LAST_8BIT_DATA ];
          }
          break;
      }
      break;
    ...
    case SPECTRUM_WIDTH :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        value = Spectrum_width_16bit_LUT[ INDEX_OF_LAST_16BIT_DATA ];
      }
      else
      {
        value = Spectrum_width_8bit_LUT[ INDEX_OF_LAST_8BIT_DATA ];
      }
      break;
    ...
    case DIFF_REFLECTIVITY :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        value = Diff_reflectivity_16bit_LUT[ INDEX_OF_LAST_16BIT_DATA ];
      }
      else
      {
        value = Diff_reflectivity_8bit_LUT[ INDEX_OF_LAST_8BIT_DATA ];
      }
      break;
    ...
    case DIFF_PHASE :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        value = Diff_phase_16bit_LUT[ INDEX_OF_LAST_16BIT_DATA ];
      }
      else
      {
        value = Diff_phase_8bit_LUT[ INDEX_OF_LAST_8BIT_DATA ];
      }
      break;
    ...
    case DIFF_CORRELATION :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        value = Diff_correlation_16bit_LUT[ INDEX_OF_LAST_16BIT_DATA ];
      }
      else
      {
        value = Diff_correlation_8bit_LUT[ INDEX_OF_LAST_8BIT_DATA ];
      }
      break;
    ...
    default :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        value = Reflectivity_16bit_LUT[ INDEX_OF_LAST_16BIT_DATA ];
      }
      else
      {
        value = Reflectivity_8bit_LUT[ INDEX_OF_LAST_8BIT_DATA ];
      }
      break;
  }
...
void build_reflectivity_16bit_LUT( float n_offset, float n_scale )
{
  int i = 0;

  Prev_reflectivity_16bit_offset = n_offset;
  Prev_reflectivity_16bit_scale = n_scale;

  Reflectivity_16bit_LUT[LOWER_THRESHOLD_INDEX] = LOWER_MOMENT_THRESHOLD;
  Reflectivity_16bit_LUT[UPPER_THRESHOLD_INDEX] = UPPER_MOMENT_THRESHOLD;
  for( i=INDEX_OF_FIRST_DATA; i<NUM_16BIT_ELEMENTS; i++ )
  {
    Reflectivity_16bit_LUT[i] = ((float)i-Prev_reflectivity_16bit_offset)/Prev_reflectivity_16bit_scale;
  }
}
...
void build_reflectivity_8bit_LUT( float n_offset, float n_scale )
{
  ...
  Reflectivity_8bit_LUT[LOWER_THRESHOLD_INDEX] = LOWER_MOMENT_THRESHOLD;
  Reflectivity_8bit_LUT[UPPER_THRESHOLD_INDEX] = UPPER_MOMENT_THRESHOLD;
  for( i=INDEX_OF_FIRST_DATA; i<NUM_8BIT_ELEMENTS; i++ )
  {
    Reflectivity_8bit_LUT[i] = ((float)i-Prev_reflectivity_8bit_offset)/Prev_reflectivity_8bit_scale;
  }
}
...
void build_velocity_high_16bit_LUT( float n_offset, float n_scale )
{
  int i = 0;

  Prev_velocity_high_16bit_offset = n_offset;
  Prev_velocity_high_16bit_scale = n_scale;

  Velocity_high_16bit_LUT[LOWER_THRESHOLD_INDEX] = LOWER_MOMENT_THRESHOLD;
  Velocity_high_16bit_LUT[UPPER_THRESHOLD_INDEX] = UPPER_MOMENT_THRESHOLD;
  for( i=INDEX_OF_FIRST_DATA; i<NUM_16BIT_ELEMENTS; i++ )
  {
    Velocity_high_16bit_LUT[i] = (((float)i-Prev_velocity_high_16bit_offset)/Prev_velocity_high_16bit_scale)*MPS_TO_KTS;
  }
}
...
void build_velocity_high_8bit_LUT( float n_offset, float n_scale )
{
  ...
  Velocity_high_8bit_LUT[LOWER_THRESHOLD_INDEX] = LOWER_MOMENT_THRESHOLD;
  Velocity_high_8bit_LUT[UPPER_THRESHOLD_INDEX] = UPPER_MOMENT_THRESHOLD;
  for( i=INDEX_OF_FIRST_DATA; i<NUM_8BIT_ELEMENTS; i++ )
  {
    Velocity_high_8bit_LUT[i] = (((float)i-Prev_velocity_high_8bit_offset)/Prev_velocity_high_8bit_scale)*MPS_TO_KTS;
  }
}
...
void build_velocity_low_16bit_LUT( float n_offset, float n_scale )
{
  int i = 0;

  Prev_velocity_low_16bit_offset = n_offset;
  Prev_velocity_low_16bit_scale = n_scale;

  Velocity_low_16bit_LUT[LOWER_THRESHOLD_INDEX] = LOWER_MOMENT_THRESHOLD;
  Velocity_low_16bit_LUT[UPPER_THRESHOLD_INDEX] = UPPER_MOMENT_THRESHOLD;
  for( i=INDEX_OF_FIRST_DATA; i<NUM_16BIT_ELEMENTS; i++ )
  {
    Velocity_low_16bit_LUT[i] = (((float)i-Prev_velocity_low_16bit_offset)/Prev_velocity_low_16bit_scale)*MPS_TO_KTS;
  }
}
...
void build_velocity_low_8bit_LUT( float n_offset, float n_scale )
{
  ...
  Velocity_low_8bit_LUT[LOWER_THRESHOLD_INDEX] = LOWER_MOMENT_THRESHOLD;
  Velocity_low_8bit_LUT[UPPER_THRESHOLD_INDEX] = UPPER_MOMENT_THRESHOLD;
  for( i=INDEX_OF_FIRST_DATA; i<NUM_8BIT_ELEMENTS; i++ )
  {
    Velocity_low_8bit_LUT[i] = (((float)i-Prev_velocity_low_8bit_offset)/Prev_velocity_low_8bit_scale)*MPS_TO_KTS;
  }
}
...
void build_spectrum_width_16bit_LUT( float n_offset, float n_scale )
{
  int i = 0;

  Prev_spectrum_width_16bit_offset = n_offset;
  Prev_spectrum_width_16bit_scale = n_scale;

  Spectrum_width_16bit_LUT[LOWER_THRESHOLD_INDEX] = LOWER_MOMENT_THRESHOLD;
  Spectrum_width_16bit_LUT[UPPER_THRESHOLD_INDEX] = UPPER_MOMENT_THRESHOLD;
  for( i=INDEX_OF_FIRST_DATA; i<NUM_16BIT_ELEMENTS; i++ )
  {
    Spectrum_width_16bit_LUT[i] = (((float)i-Prev_spectrum_width_16bit_offset)/Prev_spectrum_width_16bit_scale)*MPS_TO_KTS;
    if( Spectrum_width_16bit_LUT[i] < 0.0 ){ Spectrum_width_16bit_LUT[i] = 0.0; }
  }
}
...
void build_spectrum_width_8bit_LUT( float n_offset, float n_scale )
{
  ...
  Spectrum_width_8bit_LUT[LOWER_THRESHOLD_INDEX] = LOWER_MOMENT_THRESHOLD;
  Spectrum_width_8bit_LUT[UPPER_THRESHOLD_INDEX] = UPPER_MOMENT_THRESHOLD;
  for( i=INDEX_OF_FIRST_DATA; i<NUM_8BIT_ELEMENTS; i++ )
  {
    Spectrum_width_8bit_LUT[i] = (((float)i-Prev_spectrum_width_8bit_offset)/Prev_spectrum_width_8bit_scale)*MPS_TO_KTS;
    if( Spectrum_width_8bit_LUT[i] < 0.0 ){ Spectrum_width_8bit_LUT[i] = 0.0; }
  }
}
...
void build_diff_reflectivity_16bit_LUT( float n_offset, float n_scale )
{
  int i = 0;

  Prev_diff_reflectivity_16bit_offset = n_offset;
  Prev_diff_reflectivity_16bit_scale = n_scale;

  Diff_reflectivity_16bit_LUT[LOWER_THRESHOLD_INDEX] = LOWER_MOMENT_THRESHOLD;
  Diff_reflectivity_16bit_LUT[UPPER_THRESHOLD_INDEX] = UPPER_MOMENT_THRESHOLD;
  for( i=INDEX_OF_FIRST_DATA; i<NUM_16BIT_ELEMENTS; i++ )
  {
    Diff_reflectivity_16bit_LUT[i] = ((float)i-Prev_diff_reflectivity_16bit_offset)/Prev_diff_reflectivity_16bit_scale;
  }
}
...
void build_diff_reflectivity_8bit_LUT( float n_offset, float n_scale )
{
  ...
  Diff_reflectivity_8bit_LUT[LOWER_THRESHOLD_INDEX] = LOWER_MOMENT_THRESHOLD;
  Diff_reflectivity_8bit_LUT[UPPER_THRESHOLD_INDEX] = UPPER_MOMENT_THRESHOLD;
  for( i=INDEX_OF_FIRST_DATA; i<NUM_8BIT_ELEMENTS; i++ )
  {
    Diff_reflectivity_8bit_LUT[i] = ((float)i-Prev_diff_reflectivity_8bit_offset)/Prev_diff_reflectivity_8bit_scale;
  }
}
...
void build_diff_phase_16bit_LUT( float n_offset, float n_scale )
{
  ...
  Diff_phase_16bit_LUT[LOWER_THRESHOLD_INDEX] = LOWER_MOMENT_THRESHOLD;
  Diff_phase_16bit_LUT[UPPER_THRESHOLD_INDEX] = UPPER_MOMENT_THRESHOLD;
  for( i=INDEX_OF_FIRST_DATA; i<NUM_16BIT_ELEMENTS; i++ )
  {
    Diff_phase_16bit_LUT[i] = ((float)i-Prev_diff_phase_16bit_offset)/Prev_diff_phase_16bit_scale;
  }
}
...
void build_diff_phase_8bit_LUT( float n_offset, float n_scale )
{
  int i = 0;

  Prev_diff_phase_8bit_offset = n_offset;
  Prev_diff_phase_8bit_scale = n_scale;

  Diff_phase_8bit_LUT[LOWER_THRESHOLD_INDEX] = LOWER_MOMENT_THRESHOLD;
  Diff_phase_8bit_LUT[UPPER_THRESHOLD_INDEX] = UPPER_MOMENT_THRESHOLD;
  for( i=INDEX_OF_FIRST_DATA; i<NUM_8BIT_ELEMENTS; i++ )
  {
    Diff_phase_8bit_LUT[i] = ((float)i-Prev_diff_phase_8bit_offset)/Prev_diff_phase_8bit_scale;
  }
}
...
void build_diff_correlation_16bit_LUT( float n_offset, float n_scale )
{
  int i = 0;

  Prev_diff_correlation_16bit_offset = n_offset;
  Prev_diff_correlation_16bit_scale = n_scale;

  Diff_correlation_16bit_LUT[LOWER_THRESHOLD_INDEX] = LOWER_MOMENT_THRESHOLD;
  Diff_correlation_16bit_LUT[UPPER_THRESHOLD_INDEX] = UPPER_MOMENT_THRESHOLD;
  for( i=INDEX_OF_FIRST_DATA; i<NUM_16BIT_ELEMENTS; i++ )
  {
    Diff_correlation_16bit_LUT[i] = ((float)i-Prev_diff_correlation_16bit_offset)/Prev_diff_correlation_16bit_scale;
  }
}
...
void build_diff_correlation_8bit_LUT( float n_offset, float n_scale )
{
  ...
  Diff_correlation_8bit_LUT[LOWER_THRESHOLD_INDEX] = LOWER_MOMENT_THRESHOLD;
  Diff_correlation_8bit_LUT[UPPER_THRESHOLD_INDEX] = UPPER_MOMENT_THRESHOLD;
  for( i=INDEX_OF_FIRST_DATA; i<NUM_8BIT_ELEMENTS; i++ )
  {
    Diff_correlation_8bit_LUT[i] = ((float)i-Prev_diff_correlation_8bit_offset)/Prev_diff_correlation_8bit_scale;
  }
}
...
float *hci_basedata_tool_data_table_ptr( int moment )
{
  switch( moment )
  {
    case VELOCITY :
      switch( hci_basedata_tool_velocity_resolution() )
      {
        case DOPPLER_RESOLUTION_LOW :
          if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
          {
            return &Velocity_low_16bit_LUT[0];
          }
          else
          {
        ...
        default:
          if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
          {
            return &Velocity_high_16bit_LUT[0];
          }
          else
          {
    ...
    case SPECTRUM_WIDTH :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        return &Spectrum_width_16bit_LUT[0];
      }
      else
      {
    ...
    case DIFF_REFLECTIVITY :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        return &Diff_reflectivity_16bit_LUT[0];
      }
      else
      {
    ...
    case DIFF_CORRELATION :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        return &Diff_correlation_16bit_LUT[0];
      }
      else
      {
    ...
    default :
      if( hci_basedata_tool_data_word_size( moment ) == IS_16BIT_DATA )
      {
        return &Reflectivity_16bit_LUT[0];
      }
      else
      {

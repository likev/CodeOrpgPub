 ...
 * $Author: ccalvert $
 * $Locker:  $
 * $Date: 2003/12/18 23:04:16 $
 * $Id: recclprods_dop.c,v 1.4 2003/12/18 23:04:16 ccalvert Exp $
 * $Revision: 1.4 $
 * $State: Exp $
 */
/*******************************************************************************
Module:        recclprods_dop.c

Description:   Doppler product generation driver for cpc004/tsk007 for
...
/*******************************************************************************
Description:      generate_dop_output is the main generation module for the
                  creation of product 133, the clutter likelihood reflectivity
...
Notes:            none
*******************************************************************************/
int generate_dop_output(char *inbuf, char *outbuf,int max_bufsize) {
   ...
   RPGC_log_msg( GL_INFO,"REC dop:  begin creating product for volume: %d\n",
   ...
   RPGC_prod_desc_block((void*)outbuf,RECCLDOP,phdr->volume_scan_num);
   ...
   if(DEBUG)
      fprintf(stderr,"REC DOP  calculated radials=%d   prod 299 radials=%d\n",
              num_radials,phdr->num_radials);
#if 0
   /* array bounds quality control routines                                   */
   if(num_radials!=phdr->num_radials) {
      fprintf(stderr,"RECCLPROD_DOP ERROR: num_radial correlation error\n");
      fprintf(stderr,"  calculated radials=%d   prod 299 radials=%d\n",
         num_radials,phdr->num_radials);
      return(REC_FAILURE);
      }
#endif
   if(phdr->num_radials>=MAX_RADIALS) {
      RPGC_log_msg( GL_ERROR,
                    "RECCLPROD_DOP ERROR: num_radials is greater than MAX: %d\n",
                    phdr->num_radials);
      return(REC_FAILURE);
      }
   ...
   for(i=0;i<MAX_RADIALS;i++) {
      for(j=0;j<MAX_1KMBINS;j++) {
         ...
         short max=get_max_dop(&radial[k]);
         k+=4;
         radial_data[i][j]=max;
         }
      }
   ...
   result=build_symbology_layer((short*)outbuf,inbuf,phdr->num_radials,&length,
      radial_data,RECCLDOP,max_bufsize);
   
   /* save the offset for the start of the TAB block                          */
   tab_offset=length;
   
   /* step 3: enter the TAB block generator                                   */
   product_length=generate_TAB(inbuf,outbuf,tab_offset,RECCLDOP,max_bufsize);
   ...
   product_length-=120;
   result=RPGC_prod_hdr((void*)outbuf,RECCLDOP,&product_length);
   if(DEBUG)
      fprintf(stderr,"-> dop  completed product length=%d  result=%d\n",

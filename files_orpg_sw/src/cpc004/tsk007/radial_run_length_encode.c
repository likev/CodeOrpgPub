 ...
 * $Author: ccalvert $
 * $Locker:  $
 * $Date: 2003/12/18 23:04:15 $
 * $Id: radial_run_length_encode.c,v 1.3 2003/12/18 23:04:15 ccalvert Exp $
 * $Revision: 1.3 $
 * $State: Exp $
 */
/************************************************************************
...
int radial_run_length_encode(int start_angle, int angle_delta, 
   ...
   int i;                   /* Loop variable                            */
   short *clr;              /* Array used to hold color table values    */
   ...
   create_rec_color_table(clr,prod_id);
   ...
   for(inbuf_index=new_start_index;inbuf_index<=end_index;
      ...
      new_pixel=clr[inbuf[inbuf_index]];      
      if(TEST) fprintf(stderr,"Color Table Lookup for inbuf[%i]=%u is %u\n",
         inbuf_index,inbuf[inbuf_index],clr[inbuf[inbuf_index]]);
   ...
   free(clr); /* free the allocated color table array prior to returning*/

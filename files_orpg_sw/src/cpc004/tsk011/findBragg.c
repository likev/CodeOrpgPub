 ...
 * $Author: steves $
 * $Locker:  $
 * $Date: 2014/11/07 21:28:54 $
 * $Id: findBragg.c,v 1.4 2014/11/07 21:28:54 steves Exp $
 * $Revision: 1.4 $
 * $State: Exp $
 */
/*============================================================
...
int debug_flag = FALSE;
  ...
  if (params->status == END_VOL) {
	...
	channel = 1;
     ...
     if (total_bins >= bragg_cfg.min_bragg_cnt) {
	...
	if( (redundant_type == FAA_REDUNDANT)
           ...
           RPGC_log_msg( GL_STATUS | LE_RPG_INFO_MSG, 
            "ZDR Stats (Bragg) [RDA:%d]: %3.2f/%d/%3.2f/%d/%3.2f/%2.1f/%d", 
	    channel, zdr_bias, total_bins, Volume_zdr_bias[vol], bragg_cnt, zdr_iqr, z_pct, params->vcp_num);
        }
        else {
           RPGC_log_msg( GL_STATUS | LE_RPG_INFO_MSG, 
            "ZDR Stats (Bragg): %3.2f/%d/%3.2f/%d/%3.2f/%2.1f/%d", 
	    zdr_bias, total_bins, Volume_zdr_bias[vol], bragg_cnt, zdr_iqr, z_pct, params->vcp_num);
        }
     }
     else {
	...
	RPGCS_convert_radial_time ((Bragg_data.Last_zdr_bias_time),
	   ...
	   mind = (double) min;
  	   ...
  	   if( (redundant_type == FAA_REDUNDANT)
	      ...
	      RPGC_log_msg( GL_STATUS | LE_RPG_INFO_MSG,
 	       "ZDR Stats (Bragg) [RDA:%d]: Last detection %02d/%02d/%02d %02d:%02dZ %3.2f", 
	       channel, (int)mmd, (int)ddd, (int)yyd, (int)hrd, (int)mind, Bragg_data.Last_zdr_bias);
           }
           else {
	      RPGC_log_msg( GL_STATUS | LE_RPG_INFO_MSG,
 	       "ZDR Stats (Bragg): Last detection %02d/%02d/%02d %02d:%02dZ %3.2f", 
	       (int)mmd, (int)ddd, (int)yyd, (int)hrd, (int)mind, Bragg_data.Last_zdr_bias);
           }
        }
        else {

 ...
 * $Author: steves $
 * $Locker:  $
 * $Date: 2014/07/14 19:39:47 $
 * $Id: pbd_process_data.c,v 1.101 2014/07/14 19:39:47 steves Exp $
 * $Revision: 1.101 $
 * $State: Exp $
 */  
...
int PD_process_incomplete_radial ( char *rda_msg ){
   ...
   if( PBD_waveform_type == VCP_WAVEFORM_CS ){ 
      ...
      if( PBD_use_locally_defined_VCP )
         ...
         RPG_elev_cut_num = PBD_rda_rdccon[ gbd->base.elev_num - 1 ];
   ...
   /* If waveform is of contiguous Doppler with ambiguity resolution type, or 
      ...
      if( PBD_use_locally_defined_VCP )
         ...
         curr_rpg_elev_cut_num = PBD_rda_rdccon[ gbd->base.elev_num - 1 ];
...
int PD_move_data( Generic_basedata_t *gbd, char *rpg_radial ){
    ...
    int radial_index, fields_added, i;
    ...
    if( PBD_saved_ind >= 0 ){		
       ...
       /* Set the reflectivity radial date and time fields in header. */
       rpg_hd->surv_time = PBD_Z_hdr[PBD_saved_ind].time;       
       rpg_hd->surv_date = PBD_Z_hdr[PBD_saved_ind].date;       
...
static int Move_additional_data( char *rda_msg, char *rpg_radial ){
   ...
   int bin_70, size = 0, bin_offset = 0;
   float range;
   ...
   for( i = 0; i < generic_hd->base.no_of_datum; i++ ){
      ...
      /* Calculate the bin number for 70 kft.  This will be used to clip 
         the DP variables at 70 kft.  Note:  PBD_sr_70000 is set to -1 
         at start of volume or elevation.  It should never be less than
         zero here. */
      if( PBD_sr_70000 > 0 ){

         bin_70 = PBD_sr_70000 / mom->bin_size;
         if( bin_70 < max_bins )
            max_bins = bin_70;
...
int PD_process_rda_vcp_message( char *vcp_data ){
   ...
   int pos, ret, ind, vcp_num, vcp_added, size;
   int vol_scan_num, n_sails_cuts, p_rpg_cut;
   char *spt = NULL;
   float first_angle = -1.0;
   short *rdccon = NULL, *rdcvcpta = NULL;
   VCP_ICD_msg_t *c_vcp = NULL;
   RDA_RPG_message_header_t *msg_hd = (RDA_RPG_message_header_t *) vcp_data;

   static short rpg_cuts[ECUTMAX];
   static RDA_rdacnt_t RDA_data;

   /* Get the size of the message from the message header. */
   spt = vcp_data + sizeof(RDA_RPG_message_header_t);
   c_vcp = (VCP_ICD_msg_t *) spt;

   /* Make sure the size is not improbable. */
   if( (c_vcp->vcp_msg_hdr.msg_size < PBD_VCP_MIN_SIZE)
                        ||
       (c_vcp->vcp_msg_hdr.msg_size > PBD_VCP_MAX_SIZE) ){

      LE_send_msg( GL_INFO, "Improbable Size of RDA VCP Message (%d)\n",
                   c_vcp->vcp_msg_hdr.msg_size*sizeof(short) );
      return -1;

   }

   /* Initialize arrays used to store VCP information. */
   memset( &rpg_cuts[0], 0, ECUTMAX*sizeof(short) );

   /* This data should have arrived prior to start of new volume. */
   vol_scan_num = (PBD_volume_scan_number + 1) % PBD_MAX_SCANS;
   if( vol_scan_num == 0 )
      vol_scan_num = PBD_MAX_SCANS;

   /* Start populating the RDA VCP data structure. */
   RDA_data.last_entry = vol_scan_num % 2;

   RDA_data.last_message_time = (msg_hd->julian_date-1)*86400 +
                                 msg_hd->milliseconds/1000;
   RDA_data.last_entry_time = time(NULL);
   memset( &RDA_data.data[RDA_data.last_entry], 0, sizeof(RDA_vcp_info_t) );
   RDA_data.data[RDA_data.last_entry].volume_scan_number = vol_scan_num;

   /* Check if this VCP is defined.  If not, add the VCP definition. 
      This allows the RPG to playback data from a "unknown" VCP 
      without requiring the VCP configuration file. */
   vcp_added = 0;
   vcp_num = c_vcp->vcp_msg_hdr.pattern_number;

   /* A return value less than 0 indicates the VCP is not defined. */
   if( (ret = ORPGVCP_index( vcp_num )) < 0 ){
      ...
      /* Search for another VCP which has the "ORPGVCP_DATA_DEFINED_VCP" bit
         set.   If one is found, remove it.  We do this to prevent accumulating
         a number of "ORPGVCP_DATA_DEFINED_VCPs" and filling up available space. */
      ...
      /* Determine whether this is a CLEAR_AIR or PRECIPITATION VCP. */
      if( (vcp_num >= VCP_MIN_CLEAR_AIR) && (vcp_num <= VCP_MAX_CLEAR_AIR) )
         pos = Add_VCP( spt, CLEAR_AIR_MODE, ORPGVCP_RDA_DEFINED_VCP );

      else
         pos = Add_VCP( spt, PRECIPITATION_MODE, ORPGVCP_RDA_DEFINED_VCP );

      /* A non-negative return value indicates the VCP was added 
         successfully. */
      if( pos >= 0 ){

         /* Set flag to indicate VCP was added. */
         vcp_added = 1;

         /* Set pointers to be used for later copy. */
   ...
   if( !vcp_added  ){
      ...
      /* Set the global flag to indicate the RDA provided VCP
         definition should be used for information about the 
         VCP instead of the locally defined VCP information
         (provided via ORPGVCP_xxx functions. */
      PBD_use_locally_defined_VCP = 0;

      if( Verify_rdccon( (Vcp_struct *) c_vcp, rpg_cuts ) >= 0 ){

         rdccon = (short *) &rpg_cuts;
         rdcvcpta = (short *) spt;
   ...
   size = c_vcp->vcp_msg_hdr.msg_size*sizeof(short);
   memcpy( (void *) &PBD_rda_vcp_data, spt, size );
   memcpy( (void *) &PBD_rda_rdccon, rdccon, ECUTMAX*sizeof(short) );

   /* Set VCP Supplemental flags. */
   memset( &PBD_VCP_suppl_flags[0], 0, ECUTMAX*sizeof(unsigned short) );
   ...
   /* Clear the supplemental scan vcp flag.  It will be set in the following
      loop if VCP has supplemental scans. */
   PBD_supplemental_scan_vcp = 0;

   LE_send_msg( GL_INFO, "RDA VCP Definition .....\n" );
   ...
   for( ind = 0; ind < c_vcp->vcp_elev_data.number_cuts; ind++ ){
      ...
      LE_send_msg( GL_INFO, "--->Angle: %5.1f, RDA Cut #: %d, RPG Cut #: %d\n", 
                   elev_angle, ind+1, rdccon[ind] );

      /* Set waveform bit. */
      if( waveform == VCP_WAVEFORM_CS )
         PBD_VCP_suppl_flags[ind] |= RDACNT_IS_CS;

      else if( waveform == VCP_WAVEFORM_CD )
         PBD_VCP_suppl_flags[ind] |= RDACNT_IS_CD;

      else if( waveform == VCP_WAVEFORM_CDBATCH )
         PBD_VCP_suppl_flags[ind] |= RDACNT_IS_CDBATCH;

      else if( waveform == VCP_WAVEFORM_BATCH )
         PBD_VCP_suppl_flags[ind] |= RDACNT_IS_BATCH;

      else if( waveform == VCP_WAVEFORM_STP )
         PBD_VCP_suppl_flags[ind] |= RDACNT_IS_SPRT;

      /* Set phase bit. */
      if( phase == VCP_PHASE_SZ2 )
         PBD_VCP_suppl_flags[ind] |= RDACNT_IS_SZ2;

      /* Set super resolution  bit. */
      if( super_res & VCP_HALFDEG_RAD )
         PBD_VCP_suppl_flags[ind] |= RDACNT_IS_SR;
      ...
      if( (rdccon[ind] > 1) && (elev_angle == first_angle) ){
         ...
         PBD_VCP_suppl_flags[ind] |= RDACNT_SUPPL_SCAN;
         PBD_supplemental_scan_vcp = 1;

      }

      /* Count the number of SAILS cuts in the RDA VCP. */
      if( PBD_VCP_suppl_flags[ind] & RDACNT_SUPPL_SCAN ){

         if( rdccon[ind] != p_rpg_cut )
            n_sails_cuts++;

      }

      /* Prepare for next pass. */
      p_rpg_cut = rdccon[ind];

   }

   /* Set the number of SAILS cuts this volume. */
   PBD_N_sails_cuts_this_vol = n_sails_cuts;
   LE_send_msg( GL_INFO, "RDA VCP Has %d SAILS cuts\n", PBD_N_sails_cuts_this_vol );

   /* Update the SAILS status message. */
   ORPGSAILS_set_num_cuts( PBD_N_sails_cuts_this_vol );

   /* Copy the VCP data and RDCCON data to data structure. */
   if( (rdccon != NULL) && (rdcvcpta != NULL) ){

      int ind = RDA_data.last_entry;

      memcpy( &RDA_data.data[ind].rdcvcpta[0], rdcvcpta, VCPSIZ*sizeof(short) );
      memcpy( &RDA_data.data[ind].rdccon[0], rdccon, ECUTMAX*sizeof(short) );
      memcpy( &RDA_data.data[ind].suppl[0], PBD_VCP_suppl_flags, ECUTMAX*sizeof(unsigned short) );

      /* Write the data out to ORPGDAT_ADAPTATION LB. */
      ret = ORPGDA_write( ORPGDAT_ADAPTATION, (char *) &RDA_data, sizeof(RDA_rdacnt_t),
                          RDA_RDACNT );
      if( ret < 0 )
...
/**************************************************************************
   ...
   Return:
      -1 on error, pos in VCP table otherwise.
...
static int Add_VCP( char *vcp_data, int wx_mode, int where_defined ){
   ...
   /* Check to see if this VCP is already defined.  If already defined,
      ORPGVCP_add() returns negative error code. */
...
static int Verify_rdccon( Vcp_struct *vcp, short *rdccon ){
   ...
   if( rdccon[0] == 0 ){
      ...
      for( i = 0; i < vcp->n_ele; i++ ){
         ...
         /* Is this an MPDA VCP?  If not, then .... 
            Note:  For MPDA VCPs, we assume if the elevation angles
                   are the same, then the RPG elevation index is
                   also the same. */
         else if( (vcp_num < VCP_MIN_MPDA) || (vcp_num > VCP_MAX_MPDA) ){

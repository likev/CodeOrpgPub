 ...
 * $Id: pbd_process_header.c,v 1.163 2014/12/09 23:01:40 steves Exp $
 * $Revision: 1.163 $
 * $State: Exp $
 */
...
/*******************************************************************
   ...
   Notes:
      Whenever possible, use the VCP definition from the data provided
      to the RPG from the RDA.  However there are no guarantees the
      RDA sends the VCP definition prior to the start of volume.
      Furthermore old Level II data my not have metadata.
...
int PH_process_new_cut ( Generic_basedata_t *gbd,   /* RDA radial data message header */
   ...
   if( Terminate_cut == 1 ){
	...
	Number_elevations = ORPGVCP_get_num_elevations( vcp_num );
        ...
        /* Check PBD_rda_vcp_data.  If SAILS is running, the number of elevations 
           defined in the RPG locally defined VCP may be different than what the 
           RDA is processing.  The VCP definition provided to the RPG from the
           RDA prior to the start of volume should be used. */
        if( PBD_rda_vcp_data.vcp_msg_hdr.pattern_number != vcp_num ){

           LE_send_msg( GL_ERROR, "Something Potentially Wrong with PBD_rda_vcp_data\n" );
           LE_send_msg( GL_ERROR, "--->PBD_rda_vcp_data.vcp_msg_hdr.pattern_number: %d != vcp_num: %d\n",
                        PBD_rda_vcp_data.vcp_msg_hdr.pattern_number, vcp_num );

        }
        else {

           int vel_reso1 = 0, vel_reso2 = 0;

           /* Determine if the locally defined VCP has different number of cuts.
              This would happen if SAILS was running.  Also check if the velocity
              resolution is different.  This would happen if somebody changed the
              velocity increment. */
           vel_reso1 = PBD_rda_vcp_data.vcp_elev_data.doppler_res;
           vel_reso2 = vs_gsm->current_vcp_table.vel_resolution;
           if( (Number_elevations != PBD_rda_vcp_data.vcp_elev_data.number_cuts)
                                  ||
                       (vel_reso1 != vel_reso2) ){

              /* Write detected differences to log file. */
              if( vel_reso1  != vel_reso2 )
                 LE_send_msg( GL_INFO, "RDA VCP (DR: %d) differs from RPG locally stored VCP (DR: %d)\n",
                              vel_reso1, vel_reso2 );
 
              if( Number_elevations != PBD_rda_vcp_data.vcp_elev_data.number_cuts )
                 LE_send_msg( GL_INFO, "RDA VCP (#Cuts: %d) differs from RPG locally stored VCP (#Cuts: %d)\n",
                              PBD_rda_vcp_data.vcp_elev_data.number_cuts,
                              Number_elevations );

              Number_elevations = PBD_rda_vcp_data.vcp_elev_data.number_cuts;
              PBD_use_locally_defined_VCP = 0;

           }

        }

        /* Verify the number of elevations is within bounds. */
	...
	for (i = 0; i < Number_elevations; i++) {
           ...
           if( PBD_use_locally_defined_VCP ){
              ...
              /* Set the VCP supplemental flags.  This overwrites what has
                 already been set. */
              int waveform = ORPGVCP_get_waveform( vcp_num, i );
              int phase  = ORPGVCP_get_phase_type( vcp_num, i );
              int super_res = ORPGVCP_get_super_res( vcp_num, i );
              float elev_angle = ORPGVCP_get_elevation_angle( vcp_num, i );

              /* Set waveform bit. */
              if( waveform == VCP_WAVEFORM_CS )
                 PBD_VCP_suppl_flags[i] |= RDACNT_IS_CS;

              else if( waveform == VCP_WAVEFORM_CD )
                 PBD_VCP_suppl_flags[i] |= RDACNT_IS_CD;

              else if( waveform == VCP_WAVEFORM_CDBATCH )
                 PBD_VCP_suppl_flags[i] |= RDACNT_IS_CDBATCH;

              else if( waveform == VCP_WAVEFORM_BATCH )
                 PBD_VCP_suppl_flags[i] |= RDACNT_IS_BATCH;

              else if( waveform == VCP_WAVEFORM_STP )
                 PBD_VCP_suppl_flags[i] |= RDACNT_IS_SPRT;

              /* Set phase bit. */
              if( phase == VCP_PHASE_SZ2 )
                 PBD_VCP_suppl_flags[i] |= RDACNT_IS_SZ2;

              /* Set super resolution  bit. */
              if( super_res & VCP_HALFDEG_RAD )
                 PBD_VCP_suppl_flags[i] |= RDACNT_IS_SR;

              /* Until I figure out a better way to determine supplemental
                 cuts, if the elevation angle matches the lowest cut, it
                 is supplemental. */
              if( (vs_gsm->elev_index[i] > 1) && (elev_angle == Target_elevation[0]) )
                 PBD_VCP_suppl_flags[i] |= RDACNT_SUPPL_SCAN;

            }
            else{

              /* Fill in target elevation table from RDA VCP definition 
                 for this volume. */
              Target_elevation [i] = ORPGVCP_BAMS_to_deg( ORPGVCP_ELEVATION_ANGLE,
                                         PBD_rda_vcp_data.vcp_elev_data.data[i].angle );
              vs_gsm->elev_index[i] = PBD_rda_rdccon[i];

              /* If not using locally defined VCP, then the VCP supplemental flags
                 are already set. */
        ...
        if( PBD_use_locally_defined_VCP )
           ...
           *rpg_num_elevs = PBD_rda_rdccon[Number_elevations -1];
        ...
        if( (PBD_vcp_number != PBD_old_vcp_number)
           ...
           if( PBD_use_locally_defined_VCP ){
              ...
              /* Allocate temporary space for VCP data ... */
              vcp_table = (Vcp_struct *) calloc( 1, sizeof(Vcp_struct) );
              if( vcp_table != NULL ){

                 Ele_attr *ele = NULL;

                 /* Copy the data to local buffer. */
                 memcpy( vcp_table, &PBD_rda_vcp_data, sizeof(Vcp_struct) );
                 
                 /* Change the PRF sector boundaries to deg*10 because this is what is
                    expected. */
                 for( i = 0; i < vcp_table->n_ele; i++ ){

                    ele = (Ele_attr *) &vcp_table->vcp_ele[i][0];
                    ele->azi_ang_1 = (int) (ORPGVCP_BAMS_to_deg( ORPGVCP_AZIMUTH_ANGLE, ele->azi_ang_1 )*10.0);
                    ele->azi_ang_2 = (int) (ORPGVCP_BAMS_to_deg( ORPGVCP_AZIMUTH_ANGLE, ele->azi_ang_2 )*10.0);
                    ele->azi_ang_3 = (int) (ORPGVCP_BAMS_to_deg( ORPGVCP_AZIMUTH_ANGLE, ele->azi_ang_3 )*10.0);
                    
                 }

                 /* Copy the modified table to Volume Status. */
                 memcpy( (void *) &vs_gsm->current_vcp_table, 
                         (void *) vcp_table, 
                         sizeof(Vcp_struct) );
                 LE_send_msg( GL_INFO, "Updating Current VCP Data With RDA VCP Data\n" );
        ...
        /* Check if RDA Adaptation Data is available.  If so, extract the TOVER
           values.   This are needed for the RPG radial on the first surveillance
           cut when the DVEL moment block is missing.  Note:  The value in
           adaptation data is a float, the value for the radial is an integer*10. */
        tover = ORPGRDA_ADPT_get_data_value( ORPGRDA_ADPT_VELTOVER, &value ); 
        if( tover >= 0 ) 
           PBD_vel_tover = value*10.0f;

        /* Make sure the value is reasonable. */
        if( (PBD_vel_tover < 0.0f) 
                    || 
            (PBD_vel_tover > 200.0f) )
           PBD_vel_tover = 50.0;
         
        tover = ORPGRDA_ADPT_get_data_value( ORPGRDA_ADPT_SPWTOVER, &value ); 
        if( tover >= 0 )
           PBD_spw_tover = value*10.0f;
         
        /* Make sure the value is reasonable. */
        if( (PBD_spw_tover < 0.0f) 
                    || 
            (PBD_spw_tover > 200.0f) )
           PBD_spw_tover = 50.0;
    ...
    /* Did we have a start of elevation but the elevation number is 1.  Elevation 1
       should be start of volume. */
    if( (bdh->status == GOODBEL) && (bdh->elev_num == 1)){

       LE_send_msg( GL_STATUS | LE_RPG_WARN_STATUS, 
                    "Unexpected Start of Elevation With Cut Number of 1\n" );
       volume_ok = 0;

    }

    /* Volume is not OK.  Command restart of volume scan. */
    if (volume_ok == 0) {
    ...
    if( PBD_use_locally_defined_VCP )
       ...
       Rpg_elevation_index = PBD_rda_rdccon[ bdh->elev_num-1 ];

    LE_send_msg( GL_INFO, 
                 "New Cut: VSN: %2d, VCP: %3d, RDA Elev #: %3d, RPG Elev #: %3d, Elev: %6.2f, Waveform: %s, Split Cut: %2d\n",
                 PBD_volume_scan_number, vcp_num, bdh->elev_num, Rpg_elevation_index, bdh->elevation, 
                 Waveform[PBD_waveform_type], PBD_split_cut );
    ...
    /* If at the end of volume, reset the PBD_use_locally_defined_VCP flag. */
    if( bdh->status == GENDVOL )
...
int PH_process_header( Generic_basedata_t *gbd, char *rpg_basedata ){        
...
#ifdef FOR_DAN
   static int mon, day, yr, hr, min, sec;
#endif
   ...
   /* The surveillance date/time will be set to the same time as the radial.  If Doppler
      split cut, times will be set in PD_move_data(). */
   rpg_hd->surv_time = gbd->base.time;
   rpg_hd->surv_date = gbd->base.date;

   /* Volume begin time (millisec past midnight) and volume begin date
      (Modified Julian) .*/
   rpg_hd->begin_vol_time = Volume_time;
   rpg_hd->begin_vol_date = (unsigned short) Volume_date; 

#ifdef FOR_DAN
      if( gbd->base.status == GOODBVOL ){

         time_t timeval = (time_t)(rpg_hd->begin_vol_date - 1)*86400 + 
                          rpg_hd->begin_vol_time/1000;
         unix_time( &timeval, &yr, &mon, &day, &hr, &min, &sec );
         if( yr > 2000 )
            yr -= 2000;
         else
            yr -= 1900;
   ...
   if( (rpg_hd->elev_num == Number_elevations) 
      ...
      /* Record elevation angle of last elevation in volume scan. */
      PBD_last_elevation_angle = rpg_hd->target_elev;

      if( (rpg_hd->status == GOODBELLC) 
                    ||
          (rpg_hd->status == GOODBEL) )
         LE_send_msg( GL_INFO, "Last Elevation Angle in VCP: %d\n", 
                      PBD_last_elevation_angle );
   ...
   /* Set global last elevation flag. */
   PBD_last_ele_flag = rpg_hd->last_ele_flag;
...
#ifdef FOR_DAN
   fprintf( stderr, "%02d/%02d/%02d, %02d:%02d:%02d, %3d, %2d, %2d, %8.4f, %2d, %8.4f\n", 
            mon, day, yr, hr, min, sec, rpg_hd->vcp_num, rpg_hd->status, 
            rpg_hd->elev_num, rpg_hd->elevation, rpg_hd->azi_num, rpg_hd->azimuth );
#endif
   ...
   /* Set supplemental flag(s). */
   rpg_hd->suppl_flags = PBD_VCP_suppl_flags[rpg_hd->elev_num - 1];

   /* This SUPPLEMENTAL_CUT_TYPE bit is set if this VCP contains
      Supplemental scans.  The radial header suppl_flags field
      indicates if this specific elevation is a supplemental 
      cut. */
   if( PBD_supplemental_scan_vcp )
      rpg_hd->msg_type |= SUPPLEMENTAL_CUT_TYPE;
...
static int Find_start_and_delta_angles ( float azi, Base_data_header *rpg_hd,
    ...
    if( rpg_hd->start_angle >= 3600 )
       rpg_hd->start_angle -= 3600;
...
void PH_radial_accounting( Base_data_header *rpg_hd, char *rda_msg,
   ...
   if( rpg_hd->status == GOODBVOL || rpg_hd->status == GOODBEL ){
      ...
      if( rpg_hd->status == GOODBVOL && rpg_hd->elev_num == 1){
         ...
         accdata->acccalib = rpg_hd->calib_const_vol;
  ...
  /* Compare the sector number with the previous radial sector
   ...
   if( (rpg_hd->status == GOODBVOL) || (rpg_hd->status == GOODBEL) ||
      ...
      orpgevt_radial_acct.elevation = (short) vs_gsm->elevations[ rpg_hd->elev_num - 1 ];
      orpgevt_radial_acct.elev_num = (unsigned char) rpg_hd->elev_num;
      orpgevt_radial_acct.last_ele_flag = (unsigned char) rpg_hd->last_ele_flag;
      orpgevt_radial_acct.radial_status = (short) rpg_hd->status;
      orpgevt_radial_acct.n_sails_cuts = (unsigned char) vs_gsm->n_sails_cuts;
      orpgevt_radial_acct.sails_cut_seq = (unsigned char) vs_gsm->sails_cut_seq[ rpg_hd->elev_num - 1 ];
...
int PH_restart_scan( int parameter_1, int parameter_2 ){
   ...
   /* If RPG is connected to a "real" RDA and the RDA Status
      is not currently in OPERATE, then we ignore Restart 
      Volume and Restart Elevation commands. */
   if( (PBD_is_rda == PBD_IS_REAL_RDA)
                   &&
       ((status & RDA_STATUS_PLAYBACK)
                   ||
        (status != RDA_STATUS_OPERATE)) ){

      /* Is this a Restart VCP or Restart Elev command? */
      if( (parameter_1 == CRDA_RESTART_VCP) 
                      ||
          (parameter_1 == CRDA_RESTART_ELEV) ){

         LE_send_msg( GL_INFO, "Ignoring Commanded Restart Vol/Elevation Because:\n" );
         LE_send_msg( GL_INFO, "--->PBD Connected to REAL RDA AND RDA Status != OPERATE\n" );
         return 0;
...
int PH_send_rda_control_command( int elev_num, int command, int reason ){
   ...
   if( command == CRDA_RESTART_VCP ){

      /* Issue volume restart rda control command. */
      ...
      /* The elevation to restart is the lowest. */
      ...
      /* Command is for restart elevation and a restart of volume 
   ...
   if( (ret = PH_restart_scan( parameter_1, parameter_2 )) < 0 ){
      ...
      PBD_aborted_volume = PBD_volume_scan_number;
                          ...
                          ||
         ((parameter_1 == CRDA_RESTART_ELEV) && (parameter_2 == 1)) ){
...
int PH_process_restart_command( int command ){
   ...
   switch( command ){
      ...
      case RESTART_ELEVATION:
      {
         ...
         ret = PH_send_rda_control_command( PBD_current_elev_num, (int) CRDA_RESTART_ELEV,
                                            (int) PBD_ABORT_VCP_RESTART_COMMANDED );

         /* The cut to restart is the first cut ... */
...
static void Process_angle_data( Generic_basedata_t *gbd, Base_data_header *rpg_hd ){
   ...
   if ( (Delta_azimuth > az_tolerance)
      ...
      if ( Delta_azimuth > az_tolerance ){
         ...
         /* Improbable azimuth angle. */
         LE_send_msg( GL_STATUS | LE_RPG_WARN_STATUS, 
                      "Improbable Azimuth Angle (EL/AZ):  %4.1f/%6.1f",
                      rpg_hd->elevation, rpg_hd->azimuth );


      }

      /* Restart the elevation angle. */
      PH_send_rda_control_command( (int) gbd->base.elev_num,
...
static void Process_block_headers( Generic_basedata_t *gbd, Base_data_header *rpg_hd ){
   ...
   if( elev_hdr != NULL ){
      ...
      rpg_hd->calib_const_elev = rpg_hd->calib_const = elev_hdr->calib_const;
      ...
      rpg_hd->calib_const_elev = rpg_hd->calib_const = 0.0f;
   ...
   if( vol_hdr != NULL ){
      ...
      rpg_hd->height = (short) vol_hdr->height;
      ...
      rpg_hd->calib_const_vol = vol_hdr->calib_const;
      rpg_hd->sig_proc_states = 0xffff;
      ...
      rpg_hd->calib_const_vol = 0.0f;
      rpg_hd->sig_proc_states = PBD_UNDEFINED_SIG_PROC_STATE;

   }

   /* Is this major version 2 of Message 31?  If so, is the Generic_rad_t 
      "len" the correct size? */
   rpg_hd->v_calib_const = 0.0;
   if( vol_hdr->major_version >= GENERIC_RAD_DBZ0_MAJOR ){

      if( rad_hdr->len ==
                  (sizeof(Generic_rad_t) + sizeof(Generic_rad_dBZ0_t)) ){

         /* Extract the H and V dBZ0 values. */
         Generic_rad_dBZ0_t *hdr =
            (Generic_rad_dBZ0_t *) ((char *) rad_hdr + sizeof(Generic_rad_t));
         rpg_hd->calib_const = hdr->h_dBZ0;
         rpg_hd->v_calib_const = hdr->v_dBZ0;

      }

      /* Copy the signal processing states. */
      rpg_hd->sig_proc_states = vol_hdr->sig_proc_states;
      if( PBD_sig_proc_states == PBD_UNDEFINED_SIG_PROC_STATE )
...
static int Process_moment_block_headers( Generic_basedata_t *gbd, 
   ...
   if( mom != NULL ){
      ...
      rpg_hd->dop_range = 1;
      rpg_hd->dop_resolution = 1;
      rpg_hd->vel_tover = (short) PBD_vel_tover;
   ...
   if( mom != NULL ){
      ...
      rpg_hd->spw_tover = (short) PBD_spw_tover;
   ...
   if( (gbd->base.status == GOODBEL) 
      ...
      float height_ceiling = TOPHGT_KM - rda_height;

      /* Verify the height ceiling is positive ... if not, set height_ceiling
         to TOPHGT_KM. */
      if( height_ceiling < 0 ){

         height_ceiling = TOPHGT_KM;
         LE_send_msg( GL_ERROR, "Radar Height Error ... Setting Height Ceiling to %f\n", 
                      TOPHGT_KM );
      ...
      srto70 = (int) (IRRE*(sqrt( (sin_ele*sin_ele) +
                      2.0*height_ceiling/IRRE ) - sin_ele));

      PBD_sr_70000 = srto70*1000;
      max_surv_bins = (PBD_sr_70000 / rpg_hd->surv_bin_size);
...
static int Get_cut_type( int vcp_num, int elev_num ){
   ...
   if( PBD_use_locally_defined_VCP )
      ...
      PBD_waveform_type =
         (int) PBD_rda_vcp_data.vcp_elev_data.data[elev_num].waveform;
   ...
   if( PBD_waveform_type == VCP_WAVEFORM_CD )
      ...
      if( elev_num >= 1 ){
         ...
         if( PBD_use_locally_defined_VCP )
            ...
            waveform =
               (int) PBD_rda_vcp_data.vcp_elev_data.data[elev_num-1].waveform;

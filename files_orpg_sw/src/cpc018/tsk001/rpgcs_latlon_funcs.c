...
/*********************************************************************
   ...
   Returns:
      Returns 0 on success and -1 otherwise.  Currently always
      returns 0.
...
int RPGCS_azran_to_latlon( float azm, float rng, float *lat, float *lon ){
   ...
   /* First convert azran to x/y. Then convert x/y to latlon. */
   if( (RPGCS_azran_to_xy( azm, rng, &x, &y ) < 0)
                       ||
       (RPGCS_xy_to_latlon( x, y, lat, lon ) < 0) )
      return(-1);
...
int RPGCS_latlon_to_azran( float lat, float lon, float *azm, float *rng ){
   ...
   /* First convert the lat/lon to x/y. Then convert x/y to azran. */
   if( (RPGCS_latlon_to_xy( lat, lon, &x, &y ) < 0)
                        || 
       (RPGCS_xy_to_azran( x, y, azm, rng ) < 0) )
      return(-1);

/************************************************************************
 *									*
 *	Module:  security_log_extract.c					*
 *									*
 *	Description:  This module generates a security log on a node	*
 *		      and extracts it to the local machine.		*
 *									*
 ************************************************************************/
 ...
 * $Id: security_log_extract.c,v 1.1 2010/02/19 16:19:45 ccalvert Exp $
 * $Revision: 1.1 $
 * $State: Exp $
 */
...
typedef struct {
  ...
  char  security_log_file_name[ MAX_DIRECTORY_LENGTH+MAX_FILENAME_LENGTH ];
} node_info_t;

/*	Global variables.	*/

static	int	FAA_flag = NO;
static	char    Security_log_directory[ MAX_DIRECTORY_LENGTH ] = "";
...
static void Verify_security_log_directory( char * );
static void Verify_node( char * );
static void Verify_bandwidth_throttle( char * );
static void Print_usage();

/***************************************************************************
 Description: Main function for security log extraction.
 ***************************************************************************/
...
int main( int argc, char *argv [] )
{
  ...
  sprintf( Default_directory, "%s/%s", getenv( "HOME" ), "security_logs" );
  ...
  {
    Security_log( &Node_info[ Selected_node ] );
    Copy_log( &Node_info[ Selected_node ] );
    if( Print_filename_flag == YES );
    {
      printf( "%s\n", Node_info[Selected_node].security_log_file_name );
    }
  }
  else
  {
...
/***************************************************************************
 Description: Parse info passed in from command line.
 ***************************************************************************/
...
static void Parse_command_line( int argc, char *argv[] )
{
  ...
  {
    Verify_security_log_directory( user_defined_directory );
    strcpy( Security_log_directory, user_defined_directory );
  }
  else
  {
    strcpy( Security_log_directory, Default_directory );
  }
...
static void Print_usage()
{
  fprintf( stderr, "\nThis task generates a security log on one of the\n" );
  fprintf( stderr, "RPG nodes and copies to the local machine.\n\n" );
  fprintf( stderr, "The following options are valid:\n" );
  fprintf( stderr, "  -h - Print this message\n" );
  fprintf( stderr, "  -n - Node to obtain security log from\n" );
  fprintf( stderr, "       [Default: Local node]\n" );
  fprintf( stderr, "       *Choices: RPGA1, RPGA2, RPGB1, RPGB2, MSCF\n" );
  fprintf( stderr, "  -d - User-defined directory to copy security log into\n" );
  fprintf( stderr, "       [Default: ~/security_logs]\n" );
  ...
  fprintf( stderr, "  -p - Print name of security log file when finished\n" );
  fprintf( stderr, "       [Default: do not print]\n" );
  fprintf( stderr, "\n\n" );
}

/***************************************************************************
 Description: Validate security log directory passed on command line.
 ***************************************************************************/

static void Verify_security_log_directory( char *dir )
{
...
/***************************************************************************
 Description: Validate node name passed on command line.
 ***************************************************************************/
...
/***************************************************************************
 Description: Validate throttle factor passed on command line.
 ***************************************************************************/
...
/***************************************************************************
 Description: Perform security log on targeted node.
 ***************************************************************************/
...
static void Security_log()
{
  ...
  sprintf( cmd, "sudo security_log -p" );
  ...
  {
    /* Security log succeeded. */
    tok = strtok( output_buf, "\n" ); /* Set up tokenizer to strip newlines. */
    strcpy( Node_info[Selected_node].security_log_file_name, tok );
  }
}

/***************************************************************************
 Description: Copy security log to appropriate directory.
 ***************************************************************************/
...
static void Copy_log()
{
  ...
  /* Set destination file name. This is the same regardless
     if security log is local or remote. */

  sprintf( dest_filename, "%s/%s", Security_log_directory,
           Node_info[Selected_node].security_log_file_name );
  ...
  if( Node_info[ Selected_node ].is_local )
  {
    ...
    if( strcmp( Default_directory, Security_log_directory ) == 0 ){ return; }
  ...
  else
  {
    ...
    sprintf( src_filename, "%s:%s/%s", Node_info[ Selected_node ].source_ip,
             Default_directory, Node_info[Selected_node].security_log_file_name );
...
/***************************************************************************
 Description: Callback for data transfer from RMT library.
 ***************************************************************************/
...
/***************************************************************************
 Description: Initialize node information.
 ***************************************************************************/
...
/***************************************************************************
 Description: Test if nodes are detectable over the network.
 ***************************************************************************/

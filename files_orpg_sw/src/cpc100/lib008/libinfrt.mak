...
# $Id: libinfrt.mak,v 1.15 2014/03/11 18:07:55 jeffs Exp $
# $Revision: 1.15 $
# $State: Exp $
...
#
#	objects=`ar t ../lib015/$(ARCH)/libcmpt.a | sed "s/\.dbt/\.sht/"`; \
#	for i in $$objects; do \
...
Other_Libs =	../lib006/$(ARCH)/librsst.a \
		../lib005/$(ARCH)/librmtt.a \
		../lib002/$(ARCH)/libent.a \
		../lib003/$(ARCH)/libinfrlbt.a \
...
# NOTE: debug object files (.dbt) are used to build the static libs
...
$(ARCH)/libinfr.a: $(Other_Libs)
	...
	cd $(ARCH) ;\
	rm -f lib$(LIB_TARGET)t.a *.$(DBGOBJ_EXT) *.$(DBGOBJT_EXT) ;\
	ar x ../../lib006/$(ARCH)/librsst.a ;\
	ar x ../../lib005/$(ARCH)/librmtt.a ;\
	ar x ../../lib002/$(ARCH)/libent.a ;\
	ar x ../../lib003/$(ARCH)/libinfrlbt.a ;\
	ar x ../../lib007/$(ARCH)/libsupp.a ;\
	ar x ../../lib010/$(ARCH)/libmalrm.a ;\
	ar x ../../lib004/$(ARCH)/libmisc.a ;\
	ar x ../../lib016/$(ARCH)/libnet.a ;\
	ar x ../../lib019/$(ARCH)/libsmia.a ;\
	$(AR) libinfr.a *.$(DBGOBJT_EXT) ;\
	$(AR) libinfr.a *.$(DBGOBJ_EXT) ;\
	$(RANLIB) libinfr.a; mv libinfr.a libinfrt.a
...
SHRDLIBLD_SEARCHLIBS = $(SYS_LIBS) -lpthread

$(ARCH)/libinfr.$(LIB_EXT): $(Other_Libs)
	@if [ -d $(ARCH) ]; then set +x; \
	else (set -x; $(MKDIR) $(ARCH)); fi
	rm -f $(ARCH)/*.sht $(ARCH)/*.shr lib$(LIB_TARGET)t.$(LIB_EXT)
	objects=`ar t ../lib006/$(ARCH)/librsst.a | sed "s/\.dbt/\.sht/"`; \
	for i in $$objects; do \
	   cp ../lib006/$(ARCH)/$$i $(ARCH) ; \
	done
	objects=`ar t ../lib005/$(ARCH)/librmtt.a | sed "s/\.dbt/\.sht/"`; \
	for i in $$objects; do \
	   cp ../lib005/$(ARCH)/$$i $(ARCH) ; \
	done
	objects=`ar t ../lib002/$(ARCH)/libent.a | sed "s/\.dbt/\.sht/"`; \
	for i in $$objects; do \
	   cp ../lib002/$(ARCH)/$$i $(ARCH) ; \
	done
	objects=`ar t ../lib003/$(ARCH)/libinfrlbt.a | sed "s/\.dbt/\.sht/"`; \
	for i in $$objects; do \
	...
	cd $(ARCH) ;\
	objects=`ls *.sh*`; gcc -o libinfr.$(LIB_EXT) $(SHRDLIBLDFLAGS) $$objects $(SHRDLIBLD_SEARCHLIBS); mv libinfr.$(LIB_EXT) libinfrt.$(LIB_EXT)

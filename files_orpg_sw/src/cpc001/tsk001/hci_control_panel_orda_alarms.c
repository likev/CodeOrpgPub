/************************************************************************
 *									*
 *	Module:  hci_control_panel_orda_alarms.c			*
 *									*
 *	Description:  This module is used to display an active RDA	*
 ...
 * $Id: hci_control_panel_orda_alarms.c,v 1.9 2014/03/18 16:55:30 jeffs Exp $
 * $Revision: 1.9 $
 * $State: Exp $
 */
...
void hci_control_panel_orda_alarms( int force_draw )
{
      ...
      ( blanking && RS_RDA_ALARM_SUMMARY ) )
  {
    ...
    if( alarm != prev_alarm || force_draw )
    {
      ...
      /* The " COM " label is the longest (with respect to XTextWidt()),
      ...
      if( alarm & BIT_4_MASK )
      {
        ...
        object->height = height;
        object->flags = OBJECT_SELECTABLE;
        object->data = ( unsigned int ) RCV_MASK;
        indx++;
      ...
      /* If the Communication category has an active alarm, display
        a selectable label. */
      ...
      if( alarm & BIT_6_MASK )
      {
        ...
        object->height = height;
        object->flags = OBJECT_SELECTABLE;
        object->data = ( unsigned int ) COM_MASK;
        indx++;

        top_scanl = top_scanl + 3*height/2;
      }

      /* If the Signal Processor category has an active alarm,
         display a selectable label. */
      ...
      if( alarm & BIT_7_MASK )
      {
        ...
        object->height = height;
        object->flags = OBJECT_SELECTABLE;
        object->data = ( unsigned int ) SIG_MASK;
        indx++;

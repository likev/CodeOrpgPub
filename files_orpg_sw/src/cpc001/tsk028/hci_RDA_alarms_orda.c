 ...
 * $Id: hci_RDA_alarms_orda.c,v 1.23 2010/03/10 18:57:26 ccalvert Exp $
 * $Revision: 1.23 $
 * $State: Exp $
 */  
...
static  Widget  RCV_button       = (Widget) NULL;
static  Widget  CTR_button       = (Widget) NULL;
static  Widget  PED_button       = (Widget) NULL;
static  Widget  SIG_button       = (Widget) NULL;
static  Widget  COM_button       = (Widget) NULL;
...
static	int	hhmmss   = 0;  /* Previous Hhmmss */
static	int	yyyymmdd = 0;  /* Previous Yyyymmdd */
...
/*      if it isn't, assume all filters are set.        */
        ...
        if( status <= 0 )
        {
          ...
          Device_filter = COM_MASK | CTR_MASK | PED_MASK | RCV_MASK |
                        SIG_MASK | UTL_MASK | XMT_MASK;
          Select_all= 1;
        }
        else if( (Device_filter == 0) || (Device_filter != (COM_MASK | CTR_MASK | PED_MASK | RCV_MASK |
                                                            SIG_MASK | UTL_MASK | XMT_MASK)) )
        {
          Select_all = 0;
          All = 0;
...
/*      Receiver category       */
        ...
        RCV_button = XtVaCreateManagedWidget ("RCV   ",
        ...
        if (Device_filter & RCV_MASK) {

            XtVaSetValues (RCV_button,
                XmNset,         True,
                NULL);
        ...
        XtAddCallback (RCV_button,
                XmNvalueChangedCallback, device_toggle_callback,
                (XtPointer) ORPGRDA_DEVICE_RCV);

/*	ORDA Control category	*/
...
/*      Signal Processor category       */

        SIG_button = XtVaCreateManagedWidget ("SIG   ",
                xmToggleButtonWidgetClass,      device_rowcol,
                XmNforeground,          hci_get_read_color (TEXT_FOREGROUND),
                XmNbackground,          hci_get_read_color (BACKGROUND_COLOR1),
                XmNfontList,            hci_get_fontlist (LIST),
        ...
        if (Device_filter & SIG_MASK) {

            XtVaSetValues (SIG_button,
                XmNset,         True,
                NULL);
        ...
        XtAddCallback (SIG_button,
                XmNvalueChangedCallback, device_toggle_callback,
                (XtPointer) ORPGRDA_DEVICE_SIG);
...
/*      Communications category    */
        ...
        COM_button = XtVaCreateManagedWidget ("COM   ",
                xmToggleButtonWidgetClass,      device_rowcol,
                XmNforeground,          hci_get_read_color (TEXT_FOREGROUND),
                XmNbackground,          hci_get_read_color (BACKGROUND_COLOR1),
                XmNfontList,            hci_get_fontlist (LIST),
        ...
        if (Device_filter & COM_MASK) {

            XtVaSetValues (COM_button,
                XmNset,         True,
                NULL);
        ...
        XtAddCallback (COM_button,
                XmNvalueChangedCallback, device_toggle_callback,
                (XtPointer) ORPGRDA_DEVICE_COM);
...
/*	Button to filter list	*/

	button = XtVaCreateManagedWidget ("Filter",
	...
	XtAddCallback (button,
		XmNactivateCallback, filter_callback, NULL);
 ...
 *	    boxes.							*/
	    ...
	    if ((((Device_filter & RCV_MASK) && (alarm_data->device == ORPGRDA_DEVICE_RCV)) ||
		 ((Device_filter & CTR_MASK) && (alarm_data->device == ORPGRDA_DEVICE_CTR)) ||
		 ((Device_filter & PED_MASK) && (alarm_data->device == ORPGRDA_DEVICE_PED)) ||
		 ((Device_filter & SIG_MASK) && (alarm_data->device == ORPGRDA_DEVICE_SIG)) ||
		 ((Device_filter & UTL_MASK) && (alarm_data->device == ORPGRDA_DEVICE_UTL)) ||
		 ((Device_filter & COM_MASK) && (alarm_data->device == ORPGRDA_DEVICE_COM)) ||
 ...
 *	bit in the device filter variable.				*/
	...
	switch ((int) client_data) {
            ...
            case ORPGRDA_DEVICE_RCV :           /* Receiver */

                if (state->set) {

                    Device_filter = Device_filter | RCV_MASK;
		...
		break;
            ...
            case ORPGRDA_DEVICE_SIG :           /* Signal Processor */

                if (state->set) {

                    Device_filter = Device_filter | SIG_MASK;
		...
		break;
            ...
            case ORPGRDA_DEVICE_COM :           /* Communications */

                if (state->set) {

                    Device_filter = Device_filter | COM_MASK;
 ...
 *	    selecting it will clear all device bits.			*/
	    ...
	    XmStringFree (str);
            ...
            Device_filter = RCV_MASK | CTR_MASK | PED_MASK |
                            SIG_MASK | UTL_MASK | COM_MASK |
                            XMT_MASK;
...
/*	if it isn't, assume all filters are set.	*/
	...
	if( Device_filter & COM_MASK )
	{
	  XtVaSetValues( COM_button, XmNset, True, NULL );
	}
	else
	{
	  XtVaSetValues( COM_button, XmNset, False, NULL );
	}
	...
	if( Device_filter & SIG_MASK )
	{
	  XtVaSetValues( SIG_button, XmNset, True, NULL );
	}
	else
	{
	  XtVaSetValues( SIG_button, XmNset, False, NULL );
	}
	...
	if( Device_filter & RCV_MASK )
	{
	  XtVaSetValues( RCV_button, XmNset, True, NULL );
	}
	else
	{
	  XtVaSetValues( RCV_button, XmNset, False, NULL );
	}

 ...
 * $Id: hci_clutter_bypass_map_display_legacy.c,v 1.21 2010/03/10 18:46:51 ccalvert Exp $
 * $Revision: 1.21 $
 * $State: Exp $
 */
...
int Request_flag = HCI_NO_FLAG; /* bypass map data update flag */
...
RDA_bypass_map_msg_t Bypass_map;        /* local bypass map data */
...
/*	If low bandwidth, display a progress meter.			*/
  ...
  Init_status = hci_clutter_bypass_map_initialize ( ORPGRDA_LEGACY_CONFIG );
  ...
  status = ORPGDA_UN_register (ORPGDAT_CLUTTERMAP,
                           LBID_BYPASSMAP_LGCY, hci_bypass_map_updated);
  ...
  int value)
{
  ...
  azimuth1 = ((int) (((azimuth + AZINT) * 256) / 360)) * 360 / 256.0 - AZINT;
 ...
 *	individually.  However, performance would be sacrificed.	*/
  ...
  azimuth = 360.0 - AZINT;
  num_of_radials = BYPASS_MAP_RADIALS;
  ...
  for (i = 0; i < num_of_radials; i++)
  {
    ...
    for (j = 0; j < 32; j++)
    {
      ...
      for (k = 0; k < 16; k++)
      {
        ...
        value = hci_get_bypass_map_data((azimuth - AZINT), range, Segment);
 ...
 *	been initialized or has been updated by another user.		*/
  ...
  if( ( Init_status <= 0 ) && ( flag == 0 ) )
  {
    ...
    {
      Init_status = hci_clutter_bypass_map_read( LBID_BYPASSMAP_LGCY );
  ...
  {
    rc = hci_clutter_bypass_map_read( LBID_BYPASSMAP_LGCY );

 ...
 *
 * x25_proto.h of snet module
 *
 * SpiderX25
 * @(#)$Id: x25_proto.h,v 1.2 2000/11/22 16:36:25 jing Exp $
 * 
 * SpiderX25 Release 8
 */
...
1.   8-Nov-00  djb  Modified data message & added new ack message for Tx Acking
*/


#ifndef _X25_PROTO_H_
#define _X25_PROTO_H_

/*
   In order to avoid applications running above X25 in include this additional
   file, we include it here. This will avoid OEMs from having to mass edit all
   applications. All they will need to do is recompile with new headers and
   hey presto.
*/
...
#define		XL_INFO	 	2       /* Information type */
...
#define		N_Xelisten	19	/* Extended Listen request */
...
#ifdef X25_ACKS /* #1 */
#define         N_DatAck        16      /* Local Message Acking */
#endif /* X25_ACKS */

/* ---- TYPES FOR INFORMATION MESSAGES ---- */

#define		 M_CParmReref	    0
#define		 M_CParmUnref	    1
#define		 M_CircuitId	    2
...
#define	MAXHOSTNAMESIZE	(DTEMAXSIZE + 8 + (NSAPMAXSIZE * 2))
...
#ifdef X25_ACKS /* #1 */
       unsigned long    sequence;       /* Application ID for this message */
#endif /* X25_ACKS */
    
};

#ifdef X25_ACKS /* #1 */
/* Note - this structure overlays xdataf */
struct xdataackf 
{
	unsigned char	xl_type;        /* Always XL_DAT */
	unsigned char 	xl_command;     /* Always N_DataAck */
...
struct pvcattf {
	...
	uint32       	  sn_id;        /* Subnetwork identifier */
...
struct xlistenf {
	unsigned char	xl_type;	/* Always XL_CTL */
	unsigned char	xl_command;	/* Always N_Xlisten or N_Xelisten */
...
/* ------------------------------------------------------------------------
 * Maximum length (in bytes) of the subnetwork identifier
 * ------------------------------------------------------------------------ */

#define SN_ID_LEN		4		/* Max length of sn_id 	*/
#define SN_PRINT_LEN		SN_ID_LEN + 1 	/* +1 for EOS char	*/

/* ----------------------------------------------------------------- */
/* Union of NLI message types and the common header.                 */
/*------------------------------------------------------------------ */

typedef union x25_primitives {
	struct xhdrf	xhdr;	   /* common header 	             */

	struct xdataf	xdata;	   /* Data	                     */
	...
	struct xcanlisf xcanlis;   /* Cancel Listen Command/Response */

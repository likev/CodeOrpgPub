 ...
 * $Id: cvt_xdr_infra.c,v 1.9 2009/05/15 17:37:53 ccalvert Exp $
 * $Revision: 1.9 $
 * $State: Exp $
 */
...
/*------------------------------------------------------------------------
  This file is a modification of the ~/src/cpc100/lib007/orpg_xdr.c file 
  intended for Build 11.  The purpose of this file (along with 
  cvt_orpg_product.h) is to 
  1. permit CVT to be compiled and used with the latest updates from the 
     ROC (if any) to support the most current development work in generic 
     products and 
  2. permit standalone compilation without including another library.
  
  CVT uses this file instead of  ~/src/cpc100/lib007
  -----------------------------------------------------------------------*/
...
/* /////////////////////////////////////////////////////////////////////// */
/***************************************************************
    DECLARATIONS & INCLUDES COPIED FROM orpg_xdr.c  (cpc100/lib007)
****************************************************************/
#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
...
/*  For CVT, include cvt_orpg_product.h rather than orpg_product.h */
...
****************************************************************/
int cvt_RPGP_product_serialize (void *prod, char **serial_data);
int cvt_RPGP_product_deserialize (char *serial_data,
                        int size, void **prod);
int cvt_RPGP_product_free (void *prod);
...
/* /////////////////////////////////////////////////////////////////////// */
/***************************************************************
    DECLARATIONS & INCLUDES COPIED FROM rpgp_gen_prod_debug.c (cpc101/lib004)
    (Note that the print functions in rpgp_gen_prod_debug.c are
     out of date, so CVG uses functions from xdr_test.c) (cpc101/lib003)
****************************************************************/
...
/***************************************************************
    MODIFIED NAMES OF PUBLIC FUNCTIONS IN rpgp.h TO AVOID COLLISIONS
    NOTE: If needed, CVG would use prototypes from xdr_test.c 
          (the print functions is gpgp_gen_prod_debug.c are out of date)
    As of CVT 4.4, none of the debug functions are used.
****************************************************************/
...
/* /////////////////////////////////////////////////////////////////////// */
/***************************************************************
    LOCAL DECLARATIONS & INCLUDES 
****************************************************************/
int CVT_DEBUG=TRUE;  /* NOT USED */
...
/* /////////////////////////////////////////////////////////////////////// */
/***************************************************************
    FUNCTION DEFINITIONS COPIED FROM cpc100/lib007/orpg_xdr.c 
    Revision 1.5 Date 2008.01/11
    (last verified with Build 11 rel 1.2) (cpc100/lib007)
****************************************************************/
...
******************************************************************/
/*  CVT changes the name to avoid collision with ORPG library */
...
int cvt_RPGP_product_serialize (void *prod, char **serial_data) {
    ...
    *serial_data = NULL;
    ...
    ret = xdr_RPGP_product_t (&xdrs, prod);
    xdr_destroy (&xdrs);
    if (ret == 0) {
    free (buf);
    return (-1);
...
int cvt_RPGP_product_deserialize (char *serial_data, int size, void **prod) {
    XDR xdrs;
    RPGP_product_t *p;
    int ret;

    *prod = NULL;
    p = malloc (sizeof (RPGP_product_t));
    if (p == NULL)
    return (-1);
...
/* /////////////////////////////////////////////////////////////////////// */
/***************************************************************
    FUNCTION DEFINITIONS COPIED FROM xdr_test.c (cpc101/lib003)

    NOTE: If needed, CVT would use contents of xdr_test.c 
          (last verified with Build 11 rel 1.2) 
          The functions in rpgp_gen_prod_debug.c are not up to date
    AS OF CVT 4.4, none of the debebugging functions are used.
    
    IF DESIRED, SEE cvt_packet_28.c and cvg packet_28.h FOR 

 ...
 * $Id: gif_output.c,v 1.9 2009/05/15 17:52:36 ccalvert Exp $
 * $Revision: 1.9 $
 * $State: Exp $
 */
/* gif_output.c */
...
void gif_output_file_select_callback(Widget w, XtPointer client_data, XtPointer call_data)
{
    ...
    static char *helpfile = HELP_FILE_GIF_OUT;


    if( check_gif_libs(standalone_flag) == FALSE ) {
       ...
       xmstr = XmStringCreateLtoR(
           "At least one of the two libraries required for this function are missing.\n"\
           "     libgd.so, and libgif.so must be installed in /usr/lib on Linux \n"
    ...
    if(gsd->layers == NULL || gsd->history == NULL)
        return;

    d = XmCreateFileSelectionDialog(w, "choose_giffile_dialog", NULL, 0);
    
    XtVaSetValues(XtParent(d), XmNtitle, "Choose GIF Output File...", 
    ...
    XtAddCallback(d, XmNokCallback, gif_output_file_ok_callback, NULL);
    XtAddCallback(d, XmNcancelCallback, gif_output_file_cancel_callback, NULL);
...
/* //    }  else { // NORMAL LOCAL INSTALLATION */
      ...
      sprintf(gif_filename, "/usr/lib/libgif.so");

#else
      /* CVG 9.0 */
      sprintf(gd_filename, "/usr/local/lib/libgd.so"); 
      sprintf(gif_filename, "/usr/local/lib/libgif.so");
...
void output_image_to_gif(char *filename)
{
    ...
    int gif_colors[256];      /* we can handle up to 256 colors */
    int i, j, color;
    int color_found;
    XImage *the_image;
    FILE *giffile;
...
unsigned int color_not_found = 0;
    ...
    /* open an output file */
    if((giffile = fopen(filename, "w")) == NULL) {
    ...
    if(gsd==sd1) {
        for(i=0; i<global_palette_size_1; i++)
            gif_colors[i] = gdImageColorAllocate(outimage, 
...
/* CVG 9.0 - ELIMINATED THE KLUDGE OF ADDING COLOR WHITE HERE */

 
    
    /* grab a copy of the pixmap's data */
    if((the_image = XGetImage(display, gsd->pixmap, 0, 0, pwidth+barwidth, pheight, 
    ...
    for(i=0; i<the_image->height; i++)
      for(j=0; j<the_image->width; j++) {
          ...
          if(color_found==1 || palette_size==256) {
              gdImageSetPixel(outimage, j, i, gif_colors[color]);
             ...
             if(color_not_found == 1)
                ...
                color_not_found++;
              gdImageSetPixel(outimage, j, i, gif_colors[palette_size]);
